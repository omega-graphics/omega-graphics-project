...
**Stale linkage left in place for Phase 4.** `target_link_libraries(OmegaWTK_Media PUBLIC OmegaCommon png turbojpeg tiff)` and the corresponding `png turbojpeg tiff` on the `OmegaWTK` framework's link line are now redundant — OmegaCommon owns the link. Harmless because the static archives only get pulled in to satisfy unresolved references, of which OmegaWTK_Media has none after this phase. Phase 4 cleans these up. Same applies to `OmegaWTK_Core`'s direct `icuuc icudata icui18n` linkage.

**Phase 4 — flip WTK internals to the new headers. ✅ Complete.** Source-level renames across `wtk/include/` and `wtk/src/`: `Media::BitmapImage` → `OmegaCommon::Img::BitmapImage` (in `Composition/Canvas.{h,cpp}`, `UI/CanvasView.{h,cpp}`, `Widgets/Primatives.h`, `Composition/backend/BitmapTextureCache.{h,cpp}`, `UI/VideoView.cpp`, `Media/AudioVideoProcessorContext.h`); `OmegaWTK::UniString` / bare `UniString` → `OmegaCommon::UniString` (in `Composition/Canvas.{h,cpp}`, `Composition/FontEngine.h`, `Composition/backend/dx/DWriteFontEngine.cpp`, `Composition/backend/mtl/CTFontEngine.mm`, `Composition/backend/vk/HarfbuzzFontEngine.cpp`, `Media/wmf/WMFAudioVideoCapture.cpp`, `Native/win/WinDialog.cpp`, `UI/CanvasView.{h,cpp}`, `UI/UIView.Update.cpp`); `OmegaWTK::Unicode32Char` → `OmegaCommon::Unicode32Char` (in `Native/NativeEvent.h`, `Native/macos/CocoaEvent.mm`, `UI/UIView.Update.cpp`); `OmegaWTK::UnicodeChar` / bare `UnicodeChar` → `OmegaCommon::UnicodeChar` (in `Composition/backend/vk/HarfbuzzFontEngine.cpp`). Public WTK headers (Canvas.h, FontEngine.h, NativeEvent.h, AudioVideoProcessorContext.h) now directly `#include "omega-common/img.h"` and/or `"omega-common/unicode.h"` rather than picking the types up transitively through chained includes.

WTK tests updated to the new API: `wtk/tests/MediaCodecTest/MediaCodecTest.cpp` rewritten against `OmegaCommon::Img::loadFromBuffer` returning `Result<BitmapImage, std::string>` with `res.isOk()` / `res.isErr()` / `res.error()` / `res.value()`, and `Format::PNG/JPEG/TIFF` (scoped enum); `wtk/tests/TextCompositorTest/main.cpp` flipped to `OmegaCommon::UniString::fromUTF8`.

CMake linkage cleanup per §6d:
- `target_link_libraries(OmegaWTK_Core PUBLIC OmegaCommon icuuc icudata icui18n xml2)` → `... PUBLIC OmegaCommon xml2` (ICU now transitive).
- `target_link_libraries(OmegaWTK_Media PUBLIC OmegaCommon png turbojpeg tiff)` → `... PUBLIC OmegaCommon` (image libs now transitive; with codec sources moved out, `OmegaWTK_Media` only ships the audio/video backends).
- `target_link_libraries("OmegaWTK" PUBLIC "OmegaCommon" icuuc icudata icui18n xml2 png turbojpeg tiff)` → `... PUBLIC "OmegaCommon" xml2`.

ICU `EMBEDDED_LIBS` stays in WTK's framework for now — deployment-ownership shift to OmegaCommon is gated on the `add_omega_graphics_module(SHARED ...)` helper learning to embed (Phase 5 prereq per §10/§11).
...
- `wtk/src/Composition/backend/BitmapTextureCache.h` → `<omega-common/img.h>`.
- `wtk/src/Composition/backend/dx/DWriteFontEngine.cpp` → `<omega-common/unicode.h>`.
- `wtk/src/Composition/backend/mtl/CTFontEngine.mm` → `<omega-common/unicode.h>`.
- `wtk/src/Composition/Canvas.cpp` — dropped (Canvas.h already pulls in `<omega-common/img.h>` and `<omega-common/unicode.h>` directly).
- `wtk/src/Composition/backend/RenderTarget.cpp` — dropped (the file uses no image types; the include was dead code).
- `wtk/src/Native/win/WinUtils.cpp` — dropped (the file uses no unicode types; the include was dead code).

`grep` confirms zero remaining references to `Media::BitmapImage`, `Media::ImgProfile`, `Media::ImgByte`, `Media::ImgHeader`, `Media::ColorFormat`, `Media::AlphaFormat`, `Media::Format::*`, `Media::loadImageFrom*`, `OmegaWTK::UniString`, `OmegaWTK::UnicodeChar`, `OmegaWTK::Unicode32Char`, or `StatusWithObj<BitmapImage>` outside `wtk/docs/` (planning material).

**Phase 7 — loose ends. ✅ Complete.**

- `OmegaCommon::Img::AlphaFormat::Ingore` typo renamed to `Ignore` everywhere (`common/include/omega-common/img.h`, `common/src/img/PngCodec.cpp`, `common/src/img/JpegCodec.cpp`). The other typos in the same enum family (`Pallete`, `Premultipled`) are still present and tracked in the relocated extension proposal (see below) under "API hygiene".
- `wtk/docs/ImgCodec-API-Extension-Proposal.md` relocated to `common/docs/ImgCodec-API-Extension-Proposal.md` per Phase 0 §12 (preserving git history via `git mv`). The "Current State" section's file paths and AUTOMDEPS reference were updated to `common/...` paths, and a status banner at the top calls out that the design predates this refactor and needs rebasing onto the `OmegaCommon::Img` API surface (replace `OmegaWTK::Media::*` → `OmegaCommon::Img::*`, `StatusWithObj<BitmapImage>` → `Result<BitmapImage, std::string>`) before the design is acted on.
- `add_omega_graphics_module(SHARED ...)` extended to honour `EMBEDDED_LIBS` for non-FRAMEWORK SHARED modules. Per-platform handling: Win32 copies each DLL into `${CMAKE_BINARY_DIR}/bin/` next to the module's runtime output; macOS copies each dylib into `${CMAKE_BINARY_DIR}/lib/` and adds `@loader_path` to the module's `BUILD_RPATH` + `INSTALL_RPATH`; Linux copies each `.so` into `${CMAKE_BINARY_DIR}/lib/` and adds `$ORIGIN` to the rpaths. The semantic is "EMBEDDED_LIBS contains runtime-artifact paths" (.dll on Win32, .dylib on macOS, .so on Linux).
- New `ICU_RUNTIME` (in `common/CMakeLists.txt`) and `LIBXML2_RUNTIME` (in `wtk/CMakeLists.txt`) cache/local vars map to the platform-correct runtime artifact (DLL on Win32, dylib/.so on Mac/Linux). Callers pass the `_RUNTIME` var to `EMBEDDED_LIBS` rather than picking the wrong import-lib (.lib) on Win32.
- ICU embedding migrated from WTK to OmegaCommon. `OmegaCommon`'s `add_omega_graphics_module(...)` now takes `EMBEDDED_LIBS ${ICU_RUNTIME}`; the manual POST_BUILD ICU staging step in `common/CMakeLists.txt` (added during the earlier DLL-staging side-quest) was deleted in favour of the new helper-driven path. On macOS, OmegaCommon directly `add_dependencies()` on the ICU install-name / rpath fixup targets so the dylibs are corrected before EMBEDDED_LIBS copies them.
- `OmegaWTK`'s framework `EMBEDDED_LIBS` reduced to `${LIBXML2_RUNTIME}` (no more `${ICU_EXPORT}`). The manual POST_BUILD libxml2 staging on Win32 was removed (helper handles it). The `OmegaWTK_ThirdPartyInstallNames` macOS aggregator pruned to depend only on libxml2's install-name target — ICU's install-name targets now belong to OmegaCommon's dep chain.

**Out of scope (logged for future):**

- `OmegaCommon::Img::AlphaFormat::{Pallete, Premultipled}` and `OmegaCommon::Img::ColorFormat::Pallete` typos (a deliberate Phase 7 narrow scope — only `Ingore` was fixed because it was the only one that came up during the refactor).
- `OmegaVA` module split for `OmegaWTK::Media::Audio*` / `Video*` / `MediaPlaybackSession` etc. — separate plan doc.
- Versioned-`.so` embedding on Linux. The new `EMBEDDED_LIBS` handling on Linux copies the unversioned symlink target as a regular file; the actual SONAME the loader looks up (e.g. `libicuuc.so.78` for ICU) is not present in `${CMAKE_BINARY_DIR}/lib/`. Existing CMake auto-rpath to the third-party install dirs keeps dev builds working; installed/redistributable Linux deployments will need a versioned-symlink staging follow-up.
