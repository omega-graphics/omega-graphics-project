...
2. Chromium Embedded Framework: Electron/React Native uses the Chromium engine to render its ui which is web based language bridge. (I assume this would huge performance slowdown with this.) Chromium is a massive repo that was built during the browser wars and is bloated tons of extra features. (500MB+ install size). There are also limitations to the HTML spec, so customization of the application is limited by what the browser can do. 
