...
| 2.13a Linux/Wayland runtime backend selection | Wayland counterpart to §2.13: a `WaylandSurfaceHost` sibling to `X11SurfaceHost` (behind a shared `SurfaceHost` interface), a Wayland branch in the §2.14 `VKVisualTree`/binder, and **runtime windowing-system detection** (`GDK_IS_X11_DISPLAY` vs `GDK_IS_WAYLAND_DISPLAY`) so one binary serves both X11 and Wayland sessions. Lifts §2.13's `GDK_BACKEND=x11` constraint. GTE is already runtime-ready (surface creation dispatches by populated handle) — only its CMake co-define is needed. | **Done.** W1–W5 compile-verified; **W6 run-verified on both a live X11 session and a live Wayland session** (window + input + Vulkan present on each — verified by developer). `AUTO` now defaults to the BOTH co-build when both dev-package sets are present and degrades to whichever single protocol is installed (Wayland-only installs with no Xlib headers included). |
...
| 2.3a View / Widget / TreeHost focus + cursor + tooltip | Virtual focus manager, declarative cursor shape, virtual tooltip popups | **Focus (FocusManager, F1–F6 + M1): DONE. Cursor (C1): DONE. Tooltip: T1 done; T2 (custom styling + transitions) is the single remaining §2.3a item — planned.** (2026-07-09). The FocusManager is code-complete and marked complete at the per-step level (every focus step ✅ landed in the §2.3a implementation-phasing table below) and links into a clean build. Verification levels: F1–F3 Linux compile-verified (`libOmegaWTK.so` links clean); F4–F6 + M1 macOS compile+link+BasicAppTest-build-verified but **not yet runtime-exercised** — their consumers (TextInput / Modal / Popover / ContextMenu) do not exist yet; C1 + T1 macOS runtime-verified (T1 in BasicAppTest — hover a button shows a themed tooltip; T1 also closed the O1/O2 overlay-render gap, so overlays paint for the first time). **Update 2026-07-10:** TextInput v0 (Widget-Stub §4B) is now built and wired into BasicAppTest — the **first consumer** of F3 key-routing + M1 click-focus + F4 Tab-in/out — now **runtime-verified on macOS** (developer-confirmed 2026-07-10: type/edit/Tab/click-away-defocus all work). GTK shares the delivery path; Win32 key delivery **verified on Windows** (developer-confirmed 2026-07-10) — all three backends now deliver keyboard input. F5/F6 remain consumer-less. This also added the `ViewDelegate::onFocusGained`/`onFocusLost` virtuals + their dispatch in `onRecieveEvent` (`setFocus` already emitted the events; nothing routed them to a delegate callback) — the seam a focus-reactive widget needs. **Runtime testing then surfaced three latent bugs that only a keyboard consumer could expose, now fixed:** (1) **no key delivery on macOS/Win32** — the `OmegaWTKCocoaView` had no `keyDown:`/`keyUp:` and the Win32 `HWNDItem` proc handled no `WM_KEYDOWN`/`WM_KEYUP` (only GTK ever wired keys), so F3 routing had nothing to route; added key delivery to both (macOS built+verified; Win32 write-only, needs a WSL build — it also gained a `key_code_from_vk` VK→KeyCode map + ToUnicode char resolution it never had). (2) **macOS `key_code_from_ns` was mismapped for special keys** — letters were correct `kVK_` but the special/modifier block used a wrong table (Backspace `0x33`→`LeftControl`, Tab/Space/Escape/mods all off), so editing keys were dead while letters typed fine (they insert via the character, not `code`); rewrote the table with authoritative Apple `kVK_` codes. (3) **click-away did not defocus** — M1 left focus untouched when a click missed a click-focusable view; changed to `clearFocus()` on miss (standard "click outside resigns the field"; focus-neutral-control opt-out noted as a later refinement). F1: `View::FocusPolicy` + bitmask `\|`/`&` operators + free `FocusReason` enum + `setFocusPolicy`/`focusPolicy`/`isFocusable`/`isClickFocusable`/`isTabFocusable`/`isFocused`/`focus(reason)`/`blur`/`lastFocusReason` landed on `View`. F2: `FocusManager` (`FocusManager.h`/`.cpp`, new) owned by `WidgetTreeHost` (`focusManager_` + `focusManager()` accessor, ctor-constructed like `overlayHost_`); `View::focus`/`blur` retrofitted from stubs to route through the host. View→host linkage threaded by a new `View::setTreeHostRecurse` (`View::Impl::treeHost_`), called from `Widget::setTreeHostRecurse` + `View::addSubView`, mirroring `setSyncLaneRecurse`. F3: `WidgetTreeHost::dispatchInputEvent` now routes `KeyDown`/`KeyUp` to `focusManager_->focusedView()->emit(event)` (dropping the event when nothing is focused), replacing the old root-broadcast — TextInput v0 unblocked. M1: `LMouseDown` walks up from the hit view to the nearest `isClickFocusable()` ancestor and `setFocus(view, FocusReason::Mouse)` before the delegate sees the event (ring suppressed). **F4 done 2026-07-01** (macOS compile-verified — `libOmegaWTK_UI.a` + BasicAppTest build clean): `FocusManager::focusNext`/`focusPrevious` do a pre-order tree walk collecting `isTabFocusable()` views under a `root_` synced from `WidgetTreeHost::setRoot`, moving focus with wrap-around (`FocusReason::Tab`/`Backtab`; nothing-focused enters at first for Tab, last for Shift-Tab); `dispatchInputEvent` intercepts bare Tab / Shift+Tab in the `KeyDown`/`KeyUp` case *before* F3's forward-to-focused-view (so a focused TextInput can't swallow Tab) and swallows the matching Tab KeyUp. **F5 done 2026-07-01** (macOS compile + link + BasicAppTest build clean): `FocusManager::pushRestorationPoint` captures the raw `focusedView()` (null allowed) onto a LIFO `restorationStack_`; `popAndRestore` re-focuses the popped view with `FocusReason::Popup` (ring returns if it was tab-focused). Tolerant of detached/destroyed captures — a static `viewInSubtree(root_, saved)` pointer-compares (never dereferences) the saved holder against the live tree and skips restoration if it's gone; a null capture falls through to `setFocus(nullptr)` (restores the no-focus state). The current-holder deref hazard stays governed by the F2 teardown-clears-focus contract (unchanged). No consumers yet — Modal / Popover / ContextMenu (Widget-Stub Phase 6) are the callers. **F6 done 2026-07-01** (macOS compile + link + BasicAppTest build clean): `FocusManager::setTabOrder(a, b)` relocates `b` to immediately follow `a` in traversal (Qt semantics, chains compose); stored as a non-owning `a → b` link map applied lazily by a new private `buildTraversalOrder` (F4's `focusNext`/`focusPrevious` refactored onto it) — natural pre-order when no overrides (fast path), else override targets pulled up behind their live anchors with cycle guards + a stranded-view safety net. **The §2.3a Focus block (F1–F6 + M1) is complete.** **C1 done 2026-07-01** (macOS compile + link + BasicAppTest build clean): declarative `View::setCursorShape`/`cursorShape` (stored as `Core::Optional<Native::CursorShape>` in `View::Impl`, unset = inherit); the hover dispatcher in `WidgetTreeHost::dispatchInputEvent`, on a topmost-hovered-view change, walks up from the hit target to the nearest ancestor that declared a shape (CSS/Qt inheritance, Arrow fallback / background) and commits it to the single OS sink via a new friend-only `AppWindow::commitCursorShape` → `NativeWindow::setCursorShape` (already implemented in all three backends). Dispatcher is the sole writer per §2.2; no public `AppWindow` cursor API. **Runtime-verified in BasicAppTest** (2026-07-01, developer-confirmed on macOS): button row sets `PointingHand` on the enabled buttons + `NotAllowed` on the disabled one — both observed on hover, including PointingHand over the button's inner *label* (confirms the ancestor walk) and NotAllowed over the disabled button (confirms hit-test reaches disabled views). T1 (tooltip) not started. |
...
| 2.6 NativeClipboard | New subsystem | **Source-complete across all three backends. GTK compile-verified on Linux** (`libOmegaWTK.so` links clean); macOS (`CocoaClipboard.mm`, NSPasteboard) and Win32 (`WinClipboard.cpp`, CF_UNICODETEXT / CF_HDROP) source-complete but **compile-unverified off-platform**. Runtime verification deferred to a clipboard-consuming widget (TextInput). |
...
| 2.15 GTK4 Migration | Linux backend port `gtk+-3.0` → `gtk4`. GTK 4 is adopted for **windowing lifecycle + event handling only** — WTK keeps owning the Vulkan instance/device/surface/swap chain (no `GdkVulkanContext`, no GTE device adoption). The render target is a **bare toplevel `GdkSurface`** (`gdk_surface_new_toplevel`, **not** a `GtkWindow` — Option B′, see §2.15) off which `gdk_x11_surface_get_xid` / `gdk_wayland_surface_get_wl_surface` give the protocol handle, so §2.13's `X11SurfaceHost` and §2.13a's `SurfaceHost`/runtime-dispatch model **carry forward**; input comes from the surface `event` signal. | **Done (GTK-4-only; native-X11-session run-verify developer-pending).** All phases G4-1→G4-7 complete. GTK 3 fully retired (G4-7, full clean removal — `WTK_GTK_MAJOR` flag + every `#if` guard deleted, gtk4 required outright; GTK-3 `GTKAppWindow.cpp` deleted, GTK-4 file renamed in). Whole gtk backend is GTK 4 (Option B′ bare `GdkSurface`): GTKItem / VKVisualTree / GTKAppWindow + Screen / Theme / Clipboard / Dialog + full `GdkEvent`→`NativeEvent` input (G4-4); GTKMenu stubbed (Panels §B3). **BasicAppTest builds + runs + renders + takes live input** — Wayland session run-verified (motion/click/key/scroll + interactive edge-resize, swapchain OK, frame presented, 0 errors); X11 path smoke-verified via `GDK_BACKEND=x11` (Xwayland). gte `2DTest` is the bare-surface reference. **Remaining:** native-X11-session (real X server) run-verify by developer; gte-test-harness `gtk+-3.0` cleanup (separate, out of scope). |
...
| **GTK** `GTKAppWindow` — all methods + scale-change emit on `notify::scale-factor`                          | ✅ Done (GTK 4, run-verified Wayland). `GTKAppWindow::onNotifyScaleFactor` connects to the bare `GdkSurface`'s `notify::scale-factor`, gates on the **integer** scale actually changing, re-applies `wl_surface_set_buffer_scale`, and emits `WindowScaleFactorChanged` (old/new integer scale). **Fractional (non-integer) rendering was investigated and deliberately not pursued:** GDK hands a *bare* `GdkSurface` no `wp_viewport`, so committing true fractional-pixel buffers (dropping `buffer_scale`) sizes the window wrong — **developer-verified oversized**. Instead we commit an integer-scaled buffer and the compositor downscales it to the fractional output scale itself (it owns `wp_fractional_scale_v1` on the surface). So the only change WTK must react to is the integer step; the fractional part is transparent. X11 `Xft.dpi` listener still future work. |
...
- **GTK** — `GTKAppWindow` connects to `notify::scale-factor` on the bare `GdkSurface` (GTK 4; no `GtkWindow` under Option B′) and emits on the **integer** scale changing. It does **not** listen to `wp_fractional_scale_v1::preferred_scale`: a bare `GdkSurface` gets no `wp_viewport` from GDK, so WTK cannot commit true fractional-pixel buffers itself (verified: the window comes up oversized). The compositor owns the fractional downscale from WTK's integer-scaled buffer, so a sub-integer change needs no WTK reaction. `suggestedRect` is empty. X11 still needs an `XSettings` listener for `Xft/DPI` changes — out of scope for the first cut; integer scale covers the common case.
...
| Step | What lands                                                                                         | Blocks                              | Status |
|------|----------------------------------------------------------------------------------------------------|-------------------------------------|--------|
| 1    | `View::FocusPolicy` + `setFocusPolicy`/`isFocusable`/`focus(reason)`/`blur` + `FocusReason` enum   | —                                   | ✅ **Done** |
| 2    | `FocusManager` skeleton with `setFocus`/`focusedView`/`clearFocus` and FocusGained/Lost emission   | nothing (broadcast still works)     | ✅ **Done** |
| 3    | `WidgetTreeHost` routes KeyDown/KeyUp to `focusedView()`; removes broadcast                        | TextInput v0 can land               | ✅ **Done** |
| 4    | `focusNext`/`focusPrevious` tree walk + Tab/Shift-Tab interception                                 | tab-traversal across forms          | ✅ **Done 2026-07-01** |
| 5    | `pushRestorationPoint`/`popAndRestore`                                                              | Modal / Popover dismissal           | ✅ **Done 2026-07-01** |
| 6    | `setTabOrder(a, b)` override                                                                       | hand-tuned form order               | ✅ **Done 2026-07-01** |

Steps 1–3 unblock TextInput. Steps 4–6 are independent and can land in any order. **Also landed:** M1 (mouse-click focus — `LMouseDown` focuses the nearest `isClickFocusable()` ancestor with `FocusReason::Mouse`), which is not a numbered step but gates the click-focus path. **The Focus block (steps 1–6 + M1) is complete.**

> **F4 landed (2026-07-01).** `FocusManager` gained `focusNext()` / `focusPrevious()` plus a `setRoot(View *)` sync point. Traversal is a pre-order DFS (a static `collectTabFocusable` in `FocusManager.cpp`, parent-before-children) over the subtree rooted at `root_`, collecting only views whose `focusPolicy()` carries the `TabFocus` bit — `isTabFocusable()` is the gate, so `NoFocus` / `ClickFocus`-only views never become Tab stops. From the current holder's index, focus moves ±1 with **wrap-around**; when the holder is absent from the set (nothing focused, or a focused overlay outside `root_`), `focusNext` enters at the first stop and `focusPrevious` at the last (Qt/browser backtab convention). Both return `false` without side effects when no tab-focusable view exists. `root_` is kept in lockstep with the host's root widget from the single `WidgetTreeHost::setRoot` assignment (`&widget->viewRef()`, or `nullptr` when cleared), so traversal never walks a stale tree. Interception lives in `WidgetTreeHost::dispatchInputEvent`'s existing `KeyDown`/`KeyUp` case: a bare `KeyCode::Tab` KeyDown calls `focusPrevious()` when `modifiers.shift` else `focusNext()` and consumes the event; the matching Tab KeyUp is swallowed. This happens **before** F3's forward-to-`focusedView()`, so a focused TextInput cannot swallow Tab and trap traversal (the plan's step-1/2-before-dispatch ordering). Deferred to later phases as specified: the `setTabCaptured(true)` opt-in for widgets that genuinely need a literal Tab (Phase 4B+), `setTabOrder` overrides (F6), and enabled/visible-state filtering of Tab stops.

> **F5 landed (2026-07-01).** `FocusManager` gained `pushRestorationPoint()` / `popAndRestore()` over a LIFO `restorationStack_` (`OmegaCommon::Vector<View *>`, null entries allowed). `pushRestorationPoint` appends the raw `currentlyFocused_` — a modal/popover/context-menu calls it just before taking focus. `popAndRestore` pops the back and re-focuses it with `FocusReason::Popup` (a keyboard reason, so the ring re-appears if the view was tab-focused before the popup covered it). **Detached-capture tolerance** is the crux: a static `viewInSubtree(root_, saved)` searches the live tree by *pointer comparison only* — `saved` is never dereferenced — so a capture whose owning subtree was torn down while the popup was up fails to match and restoration is skipped (focus left as the popup left it), rather than dereferencing freed memory. A `nullptr` capture (nothing was focused at push) falls through to `setFocus(nullptr, …)`, restoring the no-focus state. Stack entries are validated at pop time, not eagerly pruned on view destruction, so a stale entry deep in the stack is simply skipped when it surfaces. **Scope boundary:** the tolerance covers the *saved* pointer only; the current holder is still governed by the F2 teardown-clears-focus contract (a caller destroying the focused view must `clearFocus()` first). There is one residual **ABA caveat** — if a View is freed and a different View is later allocated at the same address and re-attached under `root_` before the pop, `viewInSubtree` false-positives; WTK has no weak-view tracking (Chromium's `ViewTracker` solves this) and the tree walk is the pragmatic guard. **No consumers yet:** Modal / Popover / ContextMenu (Widget-Stub Phase 6) are the intended callers; F5 ships the primitives.

> **F6 landed (2026-07-01).** `FocusManager::setTabOrder(a, b)` implements the Qt `QWidget::setTabOrder` contract — `b` is relocated to immediately follow `a` in traversal regardless of tree position, and calls compose into chains (`setTabOrder(a,b)` + `setTabOrder(b,c)` → a→b→c). Storage is a raw `a → b` link map (`OmegaCommon::MapVec<View *, View *>`, non-owning, last-writer-wins per anchor); nonsensical calls (`a`/`b` null, or `a == b`) are dropped. **Applied lazily, not eagerly:** F4's `focusNext`/`focusPrevious` were refactored onto a new private `buildTraversalOrder`, which first collects the natural pre-order list, then — only when overrides exist — projects the stored links onto the *live* tab-focusable set (a link naming a detached / non-tab-focusable endpoint is ignored, pointers compared never dereferenced), marks each override target "claimed", and emits in natural order while following each chain head to completion so claimed views are pulled up behind their anchor. A `visited` set guards cycles and double-emission, and a final safety net appends any view stranded by a pure override cycle (`setTabOrder(a,b)` + `setTabOrder(b,a)`) so a pathological config can never make a focusable view unreachable. With no overrides registered `buildTraversalOrder` returns the F4 pre-order list unchanged (fast path), so verified F4 behavior is untouched. Same lifetime caveat as F5: links are pointer-keyed and inert when their views leave the tree, but stale entries linger in the map (no weak-view tracking, no `clearTabOrder` in this cut). **This closes the Focus block (§2.3a Focus, steps 1–6 + M1).**
...
> **C1 landed (2026-07-01).** Declarative side: `View::setCursorShape(CursorShape)` / `cursorShape()` on `View`, backed by a `Core::Optional<Native::CursorShape>` in `View::Impl` (unset = "no opinion", the default). The getter returns the view's own shape or `Arrow` when unset; it does **not** resolve up the ancestor chain — that is the dispatcher's job. `CursorShape` is forward-declared as an opaque `enum class ... : int` in `View.h` so the public header does not drag in `NativeWindow.h` (and its X11-macro `#undef` dance); `ViewImpl.h` / `View.Core.cpp` include the full definition. Commit side: the hover-change branch of `WidgetTreeHost::dispatchInputEvent` (right after it emits `CursorEnter`/`CursorExit`) resolves the effective shape by walking `target → parent_ptr` to the first ancestor whose optional is set (CSS/Qt cursor inheritance; `Arrow` when the chain is empty or the cursor is over the window background), then commits via a new **friend-only** `AppWindow::commitCursorShape(CursorShape)` → `NativeWindow::setCursorShape`. The friend-only placement honors §2.2's "cursor is not exposed on `AppWindow`" — the dispatcher is the sole writer; there is no public app-facing cursor setter, and the walk uses the same `View::Impl` friend seam as M1's click-focus parent walk. The three backend `setCursorShape` impls (`CocoaAppWindow` `[NSCursor set]`, `WinAppWindow` `SetCursor`, `GTKAppWindow` `gdk`) were already in place from §2.2. **Commit cadence:** only on a topmost-hovered-view *change* (per the spec) — two adjacent views with the same shape re-commit that shape once at the boundary, which is harmless; no per-move churn, no last-committed cache (deferred as unnecessary). **Runtime-verified (2026-07-01, developer-confirmed on macOS):** the BasicAppTest button row sets `PointingHand` on the three enabled buttons and `NotAllowed` on the disabled one. Both shapes appear on hover. Two behaviors the demo pins down: (1) PointingHand shows even when the cursor is over the button's inner *label* view — the ancestor walk resolves the leaf hit up to the Button's root view that declared the shape; (2) the disabled button shows `NotAllowed`, i.e. **`hitTest` reaches disabled views** (enabled-state gates event *delivery* / pseudo-classes, not hit-testing) — worth knowing for any future "disabled controls opt out of cursor" decision. The demo lives on as a standing visual check, matching BasicAppTest's role for the other §2.3a / Tier-D features. Cursor state joins `hoveredView_` as a raw-`View*`-adjacent lifetime holder in scope for the [Weak-View-Tracking-Plan](future/Weak-View-Tracking-Plan.md) sweep, not addressed here.
...
§2.3a is three logically-distinct surfaces glued together by one dispatcher (`WidgetTreeHost`). Each surface fits the AGENTS.md ~300-LOC small-feature ceiling on its own, but landing all of them in one pass would mix unrelated review surfaces. The phasing below splits §2.3a into nine small phases — six for Focus (matching the §"Focus → Implementation order" table verbatim), one for Cursor, one for the mouse-click→focus glue that depends on both Focus F1/F2 and the input dispatcher, and one for Tooltip v0. **Cross-plan prerequisites now all landed** ([Overlay-Z-Order-Plan O1–O3](Overlay-Z-Order-Plan.md#9-phases) — present/paint/dismiss, O3 shipped 2026-07-09 — and [§2.4 `NativeTimer`](#24-nativeapp--lifecycle-arguments-timers)) — T1 wires straight into them. (T1 needs O3's dismissal path, not just O1/O2 hosting + paint: the tooltip is dismissed on click / key / Widget-change.)
...
Tooltip chain:      T1 (independent — uses Overlay O1–O3 + §2.4 NativeTimer, all shipped 2026-07-09)
```
...
- **T1** ✅: hovering a Widget with `setTooltip("hello")` for ≥500ms shows a themed text overlay near the cursor; moving the cursor to a different Widget cancels and (re)starts the timer; pressing a mouse button or key dismisses immediately. (Runtime-verified in BasicAppTest 2026-07-09.)
...
##### T1 landed (2026-07-09) — runtime-verified in BasicAppTest

Tooltip v0 shipped and was confirmed on macOS: hovering a button in BasicAppTest for ~500ms shows a themed text box near the cursor; moving off / to another button / clicking / pressing a key dismisses it. Four points where the landed code diverged from or extended the T1 row:

1. **Tooltip content is UI-layer, not a Widgets-library `Label`.** The T1 row said "present a one-`Label::Create(text)` Widget," but the dispatcher lives in `OmegaWTK_UI`, which has **no dependency on `OmegaWTK_Widgets`** (verified: zero UI-source includes of a `Widgets/` header). Building a `Label` there would invert the intended Widgets→UI layering. Instead the tooltip content is a private `TooltipContentWidget` in `WidgetTreeHost.cpp` — a `UIView` with a rounded-rect `bg` element and a centered `label` text element, styled from `queryCurrentTheme()` (`controlBackground` fill, `separator` hairline border, `foreground` text). No new UI→Widgets edge.

2. **Closed an O1/O2 overlay-render gap.** Overlays had never actually rendered — O1/O2 were verified mechanically only. `OverlayHost::present` marked the overlay dirty but never gave its Views a window render target or threaded the tree host, so `FrameBuilder::buildFrame` could not rasterize them. T1 added `WidgetTreeHost::mountOverlay` / `unmountOverlay` (propagate `windowRenderTarget_` + `setTreeHostRecurse`, mirroring `initWidgetTree`), called from `present` / every dismiss path (via a new `OverlayHost::Impl::teardownEntry`). **This is what makes *any* overlay — not just tooltips — paint**, so it retroactively validates the O2 paint walk for the first time.

3. **Content is authored in the widget constructor, not `onMount`.** A presented overlay is never mounted through the `Container::addChild` path, so `onMount` never fires. `TooltipContentWidget` builds its element list + style in its constructor; `present`'s `markDirty(Style|Layout|Paint)` drives the first `buildFrame`.

4. **Default Label font size + calibrated box sizing.** The tooltip sets *no* explicit font, so its text renders at the same body default as any `Label` (theme `typography.defaultSize`) — the developer's chosen look (a first pass with an explicit ~9pt font read too small). Box width is a calibrated one-line heuristic keyed to that size (`~0.72·pt` per glyph + 8px side padding, clamped 40–420dp); height is one line + vertical padding. **This heuristic is the piece T2 replaces with real text measurement** — a long string can still overrun the clamp. `String`→`UString` widening for the text element is ASCII-only (mirrors Button's `iconToken`); full UTF-8 decoding is a T2 item.

Files as landed match the T1 row **plus** `wtk/src/UI/OverlayHost.cpp` (the mount/unmount teardown wiring) and `wtk/src/UI/OverlayHost.h` was untouched. Estimate held (~230 LOC + ~30 for the overlay-mount gap).

##### T2 — Tooltip v2 (custom styling + transitions)

T2 turns the v0 hint into a fully customizable, animated tooltip. It is the formalization of the "Customization — follow-up, not v0" sketch above, plus two items T1 surfaced (real sizing, smooth appear/disappear). Nothing in T2 changes the one-argument `setTooltip(text)` call — every knob is opt-in through the descriptor overload, and the transition has a sensible default.

**T2.a — `TooltipDesc` overload + placement + per-tooltip style.** Land the descriptor sketched above:

```cpp
enum class TooltipPlacement : uint8_t {
    Cursor, AboveWidget, BelowWidget, LeftOfWidget, RightOfWidget, InsideWidget
};
struct TooltipDesc {
    OmegaCommon::UString     text {};
    TooltipPlacement         placement = TooltipPlacement::Cursor;
    float                    gap       = 4.f;
    Core::Optional<StylePtr> style {};        // unset → theme-synthesized (v0 chrome)
    bool                     dropShadow = true;
    Core::Optional<float>    delaySec {};     // unset → 0.5s default
};
class Widget { void setTooltip(const TooltipDesc & desc); /* + v0 overload */ };
```

Storage moves from a bare `String tooltip_` on `Widget::Impl` to a `TooltipDesc` (the v0 setter fills only `text`). The dispatcher's `presentTooltip` reads the desc: `placement` maps `Cursor` → the current `OverlayAnchor::AtPoint` path and the four edge modes → `OverlayAnchor::AtWidget` with the matching `Edge` (the overlay host already edge-clamps and the `Above↔Below` flip falls out of its clamp math); `style`, when set, is applied to the tooltip `UIView` instead of the theme-synthesized default; `dropShadow` toggles `OverlayOrnamentation::dropShadow`; `delaySec` overrides the hover timer interval.

**T2.b — real text measurement (replaces T1's heuristic).** T1 sizes the box from a glyph-count estimate, which can under- or over-shoot. T2 measures: construct the tooltip `UIView` with the text + resolved font, run `UIView::measureText(tag, availWidth)` for the wrapped height, and derive the box rect from the measured text extent (single line when it fits, wrapped when a max width is exceeded). This removes the "long string overruns the clamp" limitation and drops the magic `0.72·pt` constant. If measurement is unavailable pre-mount (style not yet resolved), fall back to the T1 heuristic. Also lands full UTF-8 `String`→`UString` decoding so non-ASCII tooltips size and render correctly.

**T2.c — smooth present / dismiss transitions.** Answers [Overlay-Z-Order-Plan §11 Q1](Overlay-Z-Order-Plan.md#11-open-questions) (animated present/dismiss — the developer answered "yes"). Add an opt-in transition to the overlay layer itself so every tier benefits, not just tooltips:

```cpp
struct OverlayPresentOptions {
    Core::Optional<SharedHandle<Composition::AnimationCurve>> transitionCurve {};
    float transitionDurationSec = 0.12f;   // 0 = snap (today's behavior)
    enum class Kind : uint8_t { Fade, FadeSlideUp, FadeScale } kind = Kind::Fade;
};
```

`OverlayHost::present` grows an optional `OverlayPresentOptions` arg; when a transition is set, the host animates the overlay root view's opacity (and, for `FadeSlideUp`, a small `+Y→0` offset) from hidden to shown via the existing `Style` transition machinery (`elementBrush` / view-opacity animation — the same keyframe-lerp path Button hover uses). Dismissal reverses it: the entry is marked *dismissing*, the reverse animation runs, and the entry is erased on completion (a pending-removal flag on `Impl::Entry` plus a completion callback, so `dismiss` becomes non-immediate when a transition is configured). The tooltip dispatcher opts into a 120ms fade by default (macOS convention); `snap` (duration 0) preserves T1 behavior for callers that want it. **Reused by Popover / Modal / ContextMenu** (Widget-Stub Phase 6) — the animation lives on `OverlayHost`, not the tooltip.

**T2 phasing.** T2.a and T2.b are independent and can land in either order (both touch only the tooltip path). T2.c is the larger piece (it touches `OverlayHost`'s dismiss lifecycle) and is independent of a/b — sequence it last so the styling/sizing work isn't blocked on the animation plumbing. Each sub-phase is well under the ~300-LOC ceiling.

**T2 verification.**
- **T2.a placement**: `setTooltip(desc)` with `placement = AboveWidget` shows the tooltip edge-centered above the widget; near the window top it flips below. A `desc.style` with a custom fill/border overrides the theme default.
- **T2.b sizing**: a long tooltip string is fully enclosed by the box (no clip) at the correct width; a non-ASCII string renders and sizes correctly.
- **T2.c transitions**: the tooltip fades in over ~120ms on present and fades out on dismiss rather than snapping; `transitionDurationSec = 0` snaps (T1 behavior); a Popover presented with `Kind::FadeSlideUp` slides up while fading in.
...
#### Implementation note (landed 2026-06-25)

Small-feature scope per AGENTS.md (each backend < 300 LOC), so no sub-phase breakdown — the interface above is the contract; this note records how it was realized.

- **Header** `wtk/include/omegaWTK/Native/NativeClipboard.h` — exactly the surface above. `get_native_clipboard()` follows the `make_native_timer` convention: declared once in the header, **defined per-platform** (no cross-platform dispatcher `.cpp`); the build's per-backend glob selects the right definition. Returns a process-wide singleton (the wrapper is stateless; the OS clipboard it fronts is a single shared resource). **v0 read/write coverage is `PlainText` + `FilePaths`**; `HTML` / `Image` are `hasType`-queryable but have no get/set accessor yet (matches the spec's interface — a follow-up adds them if a widget needs rich content).
- **GTK** `wtk/src/Native/gtk/GTKClipboard.cpp` — `gtk_clipboard_get(GDK_SELECTION_CLIPBOARD)` (the Ctrl+C/V buffer, not PRIMARY). Text via `gtk_clipboard_set_text` / `wait_for_text`; file paths via `text/uri-list` — `wait_for_uris` + `g_filename_from_uri` on read, an owned `file://` URI array served through `gtk_clipboard_set_with_data` + `gtk_selection_data_set_uris` on write (clear-callback `g_strfreev`s it; on `set_with_data` failure we free it ourselves). `hasType` maps to `wait_is_text/image/uris_available` and a `text/html` target probe. The OS clipboard is resolved lazily and guarded on a null default display, so the synchronous `wait_for_*` reads (which pump a nested loop) degrade to empty when there is no display. **Compile-verified on Linux.**
- **macOS** `wtk/src/Native/macos/CocoaClipboard.mm` — `[NSPasteboard generalPasteboard]`; `setString`/`stringForType` for text, `writeObjects:`/`readObjectsForClasses:` with `NSURL` (file-URLs-only option) for paths, `availableTypeFromArray:` for `hasType`. **Compile-unverified off-platform.**
- **Win32** `wtk/src/Native/win/WinClipboard.cpp` — `OpenClipboard`/`EmptyClipboard`/`SetClipboardData`. Text via `CF_UNICODETEXT` (UTF-8↔UTF-16 through `MultiByteToWideChar`/`WideCharToMultiByte`); file paths via `CF_HDROP` (`DragQueryFileW` on read, a `DROPFILES` header + double-NUL-terminated wide list on write); HTML via the registered `"HTML Format"`. **Compile-unverified off-platform.**

CMake needs no edit — `src/Native/{gtk,macos,win}` are globbed; a `cmake .` refresh picks up the new files.
...
1. **W1 — Build: `BOTH` mode (opt-in, no behavior change). ✅ Done (configure-verified).** Add `BOTH` to `OMEGA_LINUX_VK_BACKEND` in both CMake files; define both `WTK_NATIVE_*` + `VULKAN_TARGET_*` and link both protocol libs. Leave `AUTO` defaulting to single-protocol for now (flipped in W6). With the code still `#if/#elif`, `BOTH` does not yet *build clean* — that is the point of W2–W5; W1 is the smallest reviewable CMake-only diff and is validated by `X11` / `WAYLAND` single modes still configuring + building exactly as before.
   - **Landed:** `wtk/CMakeLists.txt` + `gte/CMakeLists.txt` backend selection refactored from one `OMEGA_LINUX_USE_WAYLAND` xor flag to two independent booleans (`OMEGA_LINUX_USE_WAYLAND` / `OMEGA_LINUX_USE_X11`); `BOTH` added to the cache `STRINGS` + help text + `AUTO`/`WAYLAND`/`X11` else-chain (with a `PKG_WAYLAND_FOUND` guard mirroring the `WAYLAND` mode). Compile defs are now emitted per-flag so `BOTH` sets all four macros. wtk links `${PKG_WAYLAND_LIBRARIES}` into `OmegaWTK_Native` **only** under `BOTH` (X11 stays transitive via GTK3, matching the §2.13-verified build); single modes add no new link.
   - **Verified:** `cmake -S . -B build -DOMEGA_LINUX_VK_BACKEND=<mode>` exits 0 for `X11` ("= X11"), `BOTH` ("= X11 + Wayland (runtime-selected co-build)"), and `AUTO` (Wayland session → "= Wayland", unchanged from prior AUTO). Single-mode emitted defines/links are byte-identical to pre-W1. No `ninja` run for `BOTH` — expected red until W2–W5 per the note above.
2. **W2 — Runtime detection helper + GTKItem de-collide. ✅ Done (compile-verified, both single modes).** Add `GTKWindowing.h`. Rename `GTKItem` accessors to protocol-explicit names behind independent `#if`s; runtime `getBinding()`. Update the two existing callers (`VKVisual`, anything reading `getBinding()`). Single-protocol builds unchanged (each still compiles exactly one accessor set).
   - **Landed:** `wtk/src/Native/private_include/NativePrivate/gtk/GTKWindowing.h` (new) — `WindowingBackend { Unknown, X11, Wayland }` + `inline detectWindowingBackend(GdkDisplay*)` keyed on `GDK_IS_WAYLAND_DISPLAY` / `GDK_IS_X11_DISPLAY`, each arm behind its own `#if` (X11-only → "X11 or Unknown", Wayland-only → "Wayland or Unknown", BOTH → full three-way). `GTKItem.h`: the `#if/#elif` GDK-header include and the colliding `getDisplay()` (returned `wl_display*` vs `Display*`) split into independent `#if`s with protocol-explicit decls — `getWaylandSurface`/`getWaylandDisplay` and `getX11Display`/`getX11Window`. `GTKItem.cpp`: accessor defs renamed + independent-`#if`'d; `getBinding()` now resolves the `GdkDisplay` and `switch`es on `detectWindowingBackend` (cases `#if`-guarded), returning the `wl_surface` (Wayland) or X11 `Window` (X11). `VKVisualTree.cpp`'s `VKVisual` wrappers retargeted to the renamed accessors (`VKVisual`'s own `#if/#elif` left for W5).
   - **Verified:** `ninja OmegaWTK_Native` exits 0 under both `X11` and `WAYLAND` configs; zero new warnings attributable to the change (128 pre-existing GTK3-deprecation warnings, identical in both modes). Build dir restored to `AUTO`.
3. **W3 — `SurfaceHost` interface + `X11SurfaceHost` adoption. ✅ Done (compile-verified, both single modes).** Add `SurfaceHost.h`; make `X11SurfaceHost` derive from it with the two `hostId`-keyed forwarders. Zero behavior change on X11 (developer-verified path preserved).
   - **Landed:** `SurfaceHost.h` (new) — abstract interface: `isRealized()` / `runOnRealize()` realize gate + `reconfigureChild(hostId, rectPx, zOrder)` / `destroyChild(hostId)` Pass-2 child API + `backend()` getter over a protected `WindowingBackend backend_` (ctor-set). `X11SurfaceHost` now `: public SurfaceHost`, both ctors pass `WindowingBackend::X11` to the base; `isRealized`/`runOnRealize` marked `override` (signatures already matched). The two new forwarders are **additive** over the existing XID-keyed registry: a new `std::unordered_map<std::uint64_t, ::Window> hostChildren_` maps `hostId` → child `::Window`; `reconfigureChild` lazily `createChildWindow`s on first call for a hostId then `reconfigureChildWindow`s, `destroyChild` `destroyChildWindow`s + drops the mapping. **None of the §2.13-verified child logic (`createChildWindow`/`reconfigureChildWindow`/`destroyChildWindow`/`restackChildren`) was touched.** On a non-X11 build both forwarders are no-ops, like the rest of the host.
   - **Verified:** `ninja OmegaWTK_Native` exits 0 under both `X11` and `WAYLAND`; warning count unchanged (128, pre-existing), zero referencing the W3 files. Build dir restored to `AUTO`.
4. **W4 — `WaylandSurfaceHost`. ✅ Done (compile-verified, Wayland mode).** New files: realize gate + root path (subsurface registry present, Pass-2-aligned, unexercised).
   - **Landed:** `wtk/src/Native/private_include/NativePrivate/gtk/WaylandSurfaceHost.h` + `wtk/src/Native/gtk/WaylandSurfaceHost.cpp` (new) — `WaylandSurfaceHost : public SurfaceHost`, ctor passes `WindowingBackend::Wayland` to the base. Structure mirrors `X11SurfaceHost` line-for-line so the sibling is built to the same contract rather than special-casing the consumer: the realize gate (`onToplevelRealized(wl_surface*)` / `isRealized` / `runOnRealize`) reuses the identical mutex + drain-once logic; the child registry is the `wl_subsurface` analog of the X11 child-window registry — `createChildSurface` (`wl_compositor.create_surface` → `wl_subcompositor.get_subsurface` → `set_desync` + `set_position`), `destroyChildSurface`, `reconfigureChildSurface`, and `restackChildren` (parent-relative `place_above` chain, ascending-z, stable). The two `hostId`-keyed `SurfaceHost` forwarders (`reconfigureChild`/`destroyChild`) lazily allocate over a `hostId → wl_surface*` map exactly as X11's forward over `hostId → ::Window`. All `wl_*` code is behind its own `#if WTK_NATIVE_WAYLAND` (with an `#else` no-arg ctor and no-op forwarders), so an X11-only build compiles the host to all no-ops — the symmetric mirror of `X11SurfaceHost`'s Wayland-build behavior. No existing file touched (no `GTKAppWindow`/`VKVisual` wiring — that is W5). Child extent flows through the §2.14 swap-chain descriptor (Wayland WSI reports no surface extent), not a protocol call; first-map/parent-commit cadence is a Pass-2 concern (risk 2) and is deferred with the unexercised registry. CMake `file(GLOB)` auto-picks the new `.cpp`; W1 already links `wayland-client` under `BOTH`.
   - **Verified:** the new object compiles clean (zero warnings/errors) under the `AUTO`→Wayland build (`OmegaWTK_Native.dir/.../WaylandSurfaceHost.cpp.o`, `ninja` exit 0). The `#else` non-Wayland gating arm is compile-checked under an `X11` config (the no-op path). Run-verification of the live root/subsurface path is deferred to W5/W6 (the consumer that constructs the host) and to §2.14 Pass 2 (the child path), matching how `X11SurfaceHost`'s child API shipped ahead of its caller.
5. **W5 — GTKAppWindow + VKVisual/binder runtime dispatch. ✅ Done (compile-verified: BOTH co-build links; X11 + Wayland single modes still link).** `GTKAppWindow` owns `unique_ptr<SurfaceHost>` and constructs by detected backend; realize handler drives the live host; the `GDK_BACKEND=x11` refusal is removed. `VKVisual` carries the backend tag; `fillDescriptor` switches at runtime. **This is the step where a `BOTH` build first compiles and runs.**
   - **Landed (3 sub-parts, all replacing the last compile-time `#if/#elif` protocol gates with runtime dispatch):**
     - `VKVisualTree.{h,cpp}` — the protocol-header include and both accessor sets split from `#if/#elif` into **independent `#if`s** so the BOTH build carries `wl_surface`/`wl_display` *and* `::Display`/`::Window` at once. `VKVisual` gains a `WindowingBackend backend_` member + `backend()` getter (ctor param). `VKVisualTree`'s ctor threads the backend through to its `VKVisual`. `make_native_visual_tree` resolves the backend once via `detectWindowingBackend(gtk_widget_get_display(gtkItem->getWidget()))` and stamps it onto the tree.
     - `VKVisualBinder.cpp` — `fillDescriptor` replaces its `#if WTK_NATIVE_WAYLAND / #elif WTK_NATIVE_X11 / #else` with a `switch(visual.backend())` (each case still `#if`-guarded so a single-protocol build only references the accessors it compiled). The Wayland case fills `wl_*` + `width`/`height` (Vulkan WSI reports no Wayland surface extent); the X11 case fills `x_*`; `default` returns false (pre-realize / unknown). GTE's `makeNativeRenderTarget` already dispatches by populated handle, so populating exactly one field set is all the co-build needs.
     - `GTKAppWindow.cpp` — `surfaceHost_` is now `std::unique_ptr<SurfaceHost>`. The ctor's old `#if WTK_NATIVE_X11` block (and its `§2.13 requires GDK_BACKEND=x11` refusal) is replaced by a `switch(detectWindowingBackend(gdkDisplay))` that constructs an `X11SurfaceHost` (from `gdk_x11_display_get_xdisplay`) or a `WaylandSurfaceHost` (from `gdk_wayland_display_get_wl_display`, root-only — no compositor/subcompositor until Pass 2). `handleRealize` switches on `surfaceHost_->backend()` and `static_cast`s to the concrete host to call its protocol-specific `onToplevelRealized(::Window` / `wl_surface*)` — exactly the downcast the §2.13a "SurfaceHost design note" prescribes (`onToplevelRealized` is not on the interface). `getSurfaceHost()` returns `SurfaceHost*`; the unused `gtk_x11_surface_host_from_native` helper + its `GTKApp.h` decl/forward-decl became the backend-agnostic `surface_host_from_native` returning `SurfaceHost*`.
   - **Verified:** `ninja BasicAppTest` links clean (0 clang errors, `libOmegaWTK.so` + `BasicAppTest` built) under all three configs — `BOTH` (the headline: both surface hosts + both `VKVisual` accessor sets compile in, runtime dispatch resolves), `X11`, and `WAYLAND` (`AUTO`). No new warnings on the five touched files. Build dir restored to `AUTO`. **Run-verification is W6** (BasicAppTest on a live X11 session and a live Wayland session — window + input + Vulkan present on each; the agent's session is Wayland, so the X11 arm needs a `GDK_BACKEND=x11` or X11-session run). The pre-existing `gte/tests/vulkan/*` failures under a Wayland-only descriptor (`x_display`/`x_window` not declared without `VULKAN_TARGET_X11`; missing `vulkan/vulkan.h` on one test) are unrelated to W5 and predate it — building the `BasicAppTest` target sidesteps them.
6. **W6 — Build + smoke test on both sessions; flip `AUTO` default. ✅ Done (run-verified, both sessions; developer-verified).** Configured + ninja'd the `BOTH` build with clang++; ran BasicAppTest on a live X11 session and a live Wayland session — window + input + Vulkan present on each. `AUTO` flipped to default to `BOTH`.
   - **Landed:** `wtk/CMakeLists.txt` + `gte/CMakeLists.txt` — added an explicit `pkg_check_modules(PKG_X11 x11)` probe (X11 is no longer assumed transitive via GTK, since Wayland-only Linux installs may ship no Xlib dev headers). `AUTO` now sets `OMEGA_LINUX_USE_WAYLAND`/`OMEGA_LINUX_USE_X11` independently from `PKG_WAYLAND_FOUND`/`PKG_X11_FOUND`: both present → BOTH co-build; only one present → that protocol; neither → `FATAL_ERROR`. The explicit `X11` and `BOTH` modes gained matching `PKG_X11_FOUND` guards mirroring the existing Wayland guard. The old `XDG_SESSION_TYPE`-based AUTO branch was removed (the binary now selects the live protocol at runtime from the `GdkDisplay`, not at configure time from the session env).
   - **Verified (agent, Wayland host with X11 packages present):** `cmake -S . -B build` exits 0 and resolves `AUTO` → "= X11 + Wayland (runtime-selected co-build)" for both OmegaGTE and OmegaWTK; `ninja BasicAppTest` links clean (0 errors, `libOmegaWTK.so` + `BasicAppTest` built). Live X11-session + Wayland-session run-verification by developer.

**Mark status:** ✅ §2.13a complete — compile-verified for `BOTH`; run-verified per session arm (X11 arm and Wayland arm both confirmed, developer-verified).
...
### 2.15 GTK4 Migration — `gtk+-3.0` → `gtk4` (GTK3 Retired)

**Status: Proposed — not started.** This is a Linux-only, backend-wide port. The macOS (`Cocoa*`) and Win32 (`Win*`) backends are untouched; everything here is under `wtk/src/Native/gtk/` plus the GTK CMake block.

**Goal:** Move the Linux backend off the end-of-life GTK 3 series onto GTK 4 **without changing who owns rendering**. GTK 4 is adopted purely for windowing lifecycle (toplevel creation/present/close) and input (the `GtkEventController` family). WTK keeps owning the Vulkan instance, physical/logical device, graphics queue, `VkSurfaceKHR`, and swap chain exactly as it does under §2.13 — GTE's surface creation (`GEVulkan.cpp:4197–4233`, dispatching on a populated Xlib/Wayland handle) is **unchanged**. The only thing that changes is *where the native handle comes from*: GTK 3's `gtk_widget_get_window()` → `GdkWindow` becomes GTK 4's `gtk_native_get_surface(GTK_NATIVE(toplevel))` → `GdkSurface`, off which `gdk_x11_surface_get_xid()` / `gdk_wayland_surface_get_wl_surface()` hand back the same protocol handle §2.13/§2.13a already feed to GTE. Because the surface-ownership model is preserved, §2.13's `X11SurfaceHost` and §2.13a's `SurfaceHost` / `WaylandSurfaceHost` / `detectWindowingBackend` / runtime binder dispatch **carry forward intact** — re-sourced from `GdkSurface`, not retired (see "Relationship to §2.13 / §2.13a" below).

> **Architectural pivot — read this first.** This migration is a *windowing + input* swap, **not** a rendering-ownership swap. WTK continues to own the `VkSurfaceKHR` and swap chain, and GTE continues to create them from a native handle — there is **no `GdkVulkanContext`, and GTE does not adopt GDK's Vulkan device.** GTK 4 is used only for what it is here to do: vend a toplevel `GdkSurface` and route input events. We do not render *through* GSK; we render Vulkan into the surface GTK's toplevel owns, the same contract §2.13 established on GTK 3. The work is therefore concentrated in the GTK-3→GTK-4 API churn (window/input/clipboard/dialog idioms), not in a renderer rewrite — and §2.13a's "GTE needs no source change" still holds. (Menus are **not** part of this churn: `NativeMenu`/`GTKMenu` are deleted by [Panels-And-Window-Customization-Plan §B3](Panels-And-Window-Customization-Plan.md#b3-custom-menus--nativemenu-removed) in favor of a virtual `MenuBar` widget — see the "Menus — out of scope" note below.)

#### Why now

GTK 3 is in maintenance-only; new display-server work (fractional scale via `wp_fractional_scale_v1`, `wp_presentation_feedback` display-link, HDR, color management) all lands in GTK 4 / GDK 4. Several deferred follow-ups already named in this proposal — §2.2's Wayland fractional-scale listener, §2.9's `wp_presentation_feedback` display-link upgrade — are GTK-4-gated. The longer the backend sits on GTK 3, the more GTK-4-only surface those follow-ups accrete against a base that can't reach them.

#### The render-surface re-solve (the crux)

GTK 4 changes how you *reach* the native surface, not who owns the Vulkan rendering into it. The toplevel still has a `GdkSurface` (one per `GtkNative`, i.e. per toplevel/popover), and on X11 that surface is still backed by a real `Window`, on Wayland by a real `wl_surface`. WTK obtains that handle through `gtk_native_get_surface()` and creates its own `VkSurfaceKHR` on it — exactly the §2.13/§2.13a contract, just sourced from `GdkSurface` instead of `GdkWindow`. Three ways to put WTK's Vulkan output on screen under GTK 4:

| Option | Mechanism | Verdict |
|--------|-----------|---------|
| **A — `GdkVulkanContext`** | `gdk_surface_create_vulkan_context(surface)` → render GTE's frame into the per-frame `VkImage` GDK vends, between `gdk_draw_context_begin_frame()` / `end_frame()`. GDK owns instance/device/queue/swap chain. | **Rejected.** Forces GTE to abandon its own `VkInstance`/`VkDevice` and adopt GDK's — a renderer-ownership inversion, gated on which device accessors a given GTK 4 minor exposes, with a device-feature/extension negotiation problem if GDK didn't enable what GTE needs. Buys nothing we need: WTK already presents fine into a surface it owns. |
| **B — `GtkNative` surface handle (from a `GtkWindow`)** | `gtk_native_get_surface(GTK_NATIVE(toplevel))` → `GdkSurface`; `gdk_x11_surface_get_xid()` / `gdk_wayland_surface_get_wl_surface()` yield the protocol handle; WTK creates the `VkSurfaceKHR` + swap chain itself (GTE unchanged). | **Superseded by B′ — fails on GTK 4.** The `GtkWindow`'s `GskRenderer` contends for the same surface → `vkCreateSwapchainKHR` returns `VK_ERROR_SURFACE_LOST_KHR` (probe finding below). GTK 3's app-paintable / double-buffer opt-out is gone in GTK 4. |
| **B′ — bare `GdkSurface` we own (chosen)** | `gdk_surface_new_toplevel(display)` → a toplevel `GdkSurface` with **no** `GtkWidget`/`GskRenderer`; same protocol-handle extraction; WTK owns the `VkSurfaceKHR`/swap chain (GTE unchanged). GTK/GDK does windowing lifecycle + input only (`event` signal). | **Chosen.** Keeps Option B's ownership model and the `SurfaceHost` / `detectWindowingBackend` apparatus, but removes the GSK contention by never binding a renderer to the surface. Probe-verified on Wayland. |
| **C — offscreen → `GdkTexture` import** | Render Vulkan offscreen to an exportable dmabuf, import zero-copy via `GdkDmabufTextureBuilder` (GTK 4.14+), paint with `gtk_snapshot_append_texture`. | **Fallback only.** Available if a future GTK 4 / compositor combination ever refuses a directly-owned child surface; not needed while §2.13a's child `Window` / `wl_subsurface` registry works under Option B. |

**Decision (revised — see probe finding below): Option B′ — a *bare* `GdkSurface` we own, not a `GtkWindow`.** The original Option B sourced the handle from a `GtkWindow`'s `GtkNative`. A probe (gte `2DTest`, ported to GTK 4) showed that does **not** work on GTK 4: a `GtkWindow`/`GtkWidget` realizes a `GskRenderer` onto its `GdkSurface`, and that GSK renderer competes with WTK's own Vulkan swap chain for the same `wl_surface` — `vkCreateSwapchainKHR` fails with `VK_ERROR_SURFACE_LOST_KHR` plus a Wayland `Error 71 (Protocol error)`, and GTK additionally tries to stand up its own GSK Vulkan device. GTK 3's escape hatch (`gtk_widget_set_app_paintable(TRUE)` + `gtk_widget_set_double_buffered(FALSE)`) is **gone in GTK 4**, so there is no way to make a `GtkWindow` keep its hands off the surface. The fix keeps the *spirit* of Option B (WTK owns the `VkSurfaceKHR`/swap chain, GTE untouched) but drops the `GtkWindow`: create the toplevel directly with `gdk_surface_new_toplevel(display)`, which has no `GtkWidget` and therefore no `GskRenderer` bound to it. GTK/GDK is still used — but only for windowing lifecycle (`gdk_toplevel_present`/close) and **input via the `GdkSurface` `event` signal**. §2.13a's child-surface registry (X11 child `Window`s, `wl_subsurface`s via `SurfaceHost`) carries forward for the §2.14-Pass-2 content-node path; Option C (offscreen → `GdkTexture` import) is held in reserve only if direct surface ownership is ever blocked.

> **Probe result (gte `2DTest` → GTK 4, developer-visually-verified).** Bare-`GdkSurface` path: size negotiated via `compute-size` + the `layout(w,h)` signal (500×500 @scale 2 → 1000×1000 px), `vkCreateSwapchainKHR` **succeeds**, frame presents, the window stays up with no `SURFACE_LOST` / protocol error / GSK Vulkan warning, and the red mesh renders correctly. Input arrives through the surface `event` signal. The earlier `GtkWindow` path is retained in git history as the negative result. Mechanics captured in the agent memory `gtk4-bare-gdksurface-no-gtkwindow`.
>
> **Note — GSK coexistence is resolved by design, not by smoke test.** Because the render target is a bare `GdkSurface` with no `GskRenderer` attached, there is no GSK content to composite over (or clobber) WTK's swap chain — the coexistence question the old Option B carried into G4-3 no longer applies. The cost is that the GTK-4 backend stops being a `GtkWindow` consumer (it owns the toplevel directly), which ripples into G4-2 (window skeleton) and G4-4 (input) below, and into §2.13/§2.13a's `GTKAppWindow`-is-a-`GtkWindow` assumption.

#### GTK3 → GTK4 inventory (what actually changes)

Every GTK-3-only idiom found in the backend today (`wtk/src/Native/gtk/`, 12 files / ~3.6k LOC) and its GTK 4 replacement:

| GTK 3 (today) | GTK 4 replacement | Files |
|---------------|-------------------|-------|
| `gtk_window_new(GTK_WINDOW_TOPLEVEL)` | `gtk_window_new()` (no arg); `gtk_application_window_new(app)` | `GTKAppWindow.cpp` |
| `gtk_container_add` / `gtk_box_pack_start` | `gtk_window_set_child` / `gtk_box_append` | `GTKAppWindow.cpp` |
| `gtk_widget_show_all` | widgets visible by default; `gtk_window_present()` | `GTKAppWindow.cpp` |
| `GdkWindow`, `gtk_widget_get_window` | `GdkSurface`, `gtk_native_get_surface(GTK_NATIVE(toplevel))` | `GTKItem.cpp`, `GTKAppWindow.cpp` |
| `gdk_x11_window_get_xid` | `gdk_x11_surface_get_xid` (`gdk/x11/gdkx.h`), off `gtk_native_get_surface()` — **the root surface handle WTK creates its `VkSurfaceKHR` on** (Wayland counterpart: `gdk_wayland_surface_get_wl_surface`) | `GTKItem.cpp`, `GTKAppWindow.cpp` |
| `gtk_drawing_area_new` + `draw` signal | no GTK render widget — WTK presents Vulkan into the toplevel `GdkSurface` directly (the §2.13 model); the root is just the toplevel | `GTKItem.cpp` |
| `gtk_widget_set_double_buffered` / `set_app_paintable` | **removed entirely** — these calls are gone in GTK 4; delete them | `GTKItem.cpp`, `GTKAppWindow.cpp` |
| `button-press-event` / `motion-notify-event` / `enter`/`leave` / `scroll-event` / `key-press-event` signals | `GtkEventController` family added via `gtk_widget_add_controller`: `GtkGestureClick`, `GtkEventControllerMotion`, `GtkEventControllerScroll`, `GtkEventControllerKey` | `GTKAppWindow.cpp` |
| `configure-event` / `size-allocate` for resize | `GdkSurface` `notify::width` / `notify::height` (+ `GtkWidget` `size_allocate` vfunc) drives WTK's own swap-chain rebuild — same resize path as §2.13's `configure-event`, different signal | `GTKAppWindow.cpp` |
| ~~`GtkMenu` / `GtkMenuBar` / `GtkMenuItem`~~ | **Not migrated — `GTKMenu.cpp` is deleted, not ported.** Menus become a virtual `MenuBar` widget per [Panels-And-Window-Customization-Plan §B3](Panels-And-Window-Customization-Plan.md#b3-custom-menus--nativemenu-removed). See the "Menus — out of scope" note below. | ~~`GTKMenu.cpp`~~ |
| `gtk_clipboard_get` + sync `wait_for_text` | `gdk_display_get_clipboard()` → `GdkClipboard`; **async only** (`gdk_clipboard_read_text_async` / `gdk_clipboard_set`) — the §2.6 sync `getText()` contract must be reconsidered (see Risk 3) | `GTKClipboard.cpp` |
| `gdk_display_get_n_monitors` / `_get_monitor` / `gdk_display_get_primary_monitor` | `gdk_display_get_monitors()` → `GListModel` (iterate `g_list_model_get_item`); **no "primary" monitor concept in GTK 4** — §2.9's GTK primary-anchoring picks first / pointer-containing monitor | `GTKScreen.cpp` |
| `GdkScreen` (`gdk_screen_*`) | **removed** — geometry/scale from `GdkMonitor`; fractional scale from `gdk_surface_get_scale()` (double, GTK 4.12+) | `GTKAppWindow.cpp`, `GTKScreen.cpp`, `GTKTheme.cpp` |
| `gtk_style_context_lookup_color` (theme colors) | `GtkStyleContext` largely deprecated in GTK 4; foreground via `gtk_widget_get_color()` (4.10+); background needs a CSS / snapshot probe — §2.5 GTK theme extraction gets harder (see Risk 4) | `GTKTheme.cpp` |
| `GtkFileChooserDialog` / `GtkMessageDialog` | deprecated in 4.10 → async `GtkFileDialog` / `GtkAlertDialog`; §2.8's GTK backend moves to the async API | `GTKDialog.cpp` |

Survives with little/no change: `GTKApp.cpp` (already `GtkApplication` + `activate`/`shutdown`/`open` — the GTK 4 model), `GTKTimer.cpp` (`g_timeout_add` is GLib, version-agnostic), `GTKNote.cpp` (`libnotify` / `GNotification` is GTK-version-independent). `GTKTheme.cpp` typography via Pango survives; only its color lookup changes.

#### Relationship to §2.13 / §2.13a

This phase **preserves the surface-ownership model** of both — it changes only the GTK API that vends the native handle:

- **§2.13 (X11 direct ownership) carries forward.** WTK still owns the X11 surface and creates the `VkSurfaceKHR` on it; `X11SurfaceHost` is unchanged. The only difference is the handle source: `gtk_native_get_surface()` → `gdk_x11_surface_get_xid()` replaces GTK 3's `gtk_widget_get_window()` → `gdk_x11_window_get_xid()`. The `app_paintable`/double-buffer-off calls simply go away (they no longer exist in GTK 4); nothing replaces them because WTK was never relying on GTK to render.
- **§2.13a (X11 + Wayland co-build) carries forward intact — this is the headline.** Its four blockers were all about WTK creating a protocol-specific `VkSurfaceKHR`, and WTK *still does that* under GTK 4. `WindowingBackend` / `detectWindowingBackend` (it already keys off `GDK_IS_X11_DISPLAY` / `GDK_IS_WAYLAND_DISPLAY`, which are GTK-4-valid), the `SurfaceHost` interface, `X11SurfaceHost`, `WaylandSurfaceHost`, and the `VKVisualBinder` runtime dispatch are all reused — the realize handler just hands them a `GdkSurface`-derived handle instead of a `GdkWindow`-derived one. §2.13a is **not** re-scoped or closed by §2.15; it is the mechanism §2.15 builds on for the co-build.
- **§2.14 (NativeVisualTree) is preserved.** `Native::VisualTree` / `Native::Visual` stay; the Linux `VKVisualTree` impl is retargeted from the `GdkWindow`-backed `GTKItem` handle to the `GdkSurface`-backed one. Pass 2 child nodes stay on §2.13a's child `Window` / `wl_subsurface` registry (Option C held in reserve only if direct child ownership is ever blocked).

#### Per-platform implementation notes (Linux only)

- **Root surface.** On the toplevel's realize, resolve `gtk_native_get_surface(GTK_NATIVE(toplevel))` → `GdkSurface`, then `gdk_x11_surface_get_xid()` / `gdk_wayland_surface_get_wl_surface()` for the protocol handle, and hand it to the live `SurfaceHost` exactly as §2.13's realize handler does today. WTK creates the `VkSurfaceKHR` + swap chain (GTE unchanged); resize is driven off `GdkSurface` `notify::width`/`height` (and the `size_allocate` vfunc), rebuilding WTK's swap chain — the same flow §2.13 ran off `configure-event`. GTK's frame clock (`gtk_widget_add_tick_callback`) is the natural §2.9 `NativeDisplayLink` source on GTK 4.
- **No GTE device adoption.** GTE keeps creating and owning its `VkInstance` / `VkPhysicalDevice` / `VkDevice` / queues on Linux, unchanged from §2.13. There is no `GdkVulkanContext` and nothing to adopt — this is the deliberate divergence from Option A and the reason the phase is windowing/input churn rather than a renderer rewrite.
- **Input.** Controllers attach to the toplevel (or the render-area widget); coordinates arrive widget-local from the gesture/motion controllers, exactly what `View::containsPoint` / the §2.3a hover dispatcher expect — same contract §2.13 established, different delivery API.
- **Menus — not migrated.** No GTK 4 menu work happens in this phase. `NativeMenu` / `GTKMenu.cpp` are removed wholesale (not ported to `GMenu` / `GtkPopoverMenuBar`) — see the note immediately below.

> **Menus — out of scope for this migration (superseded by the Panels plan).** Do **not** port `GTKMenu.cpp` to GTK 4. The entire `Native::NativeMenu` interface and its GTK backend are **deleted** by [Panels-And-Window-Customization-Plan §B3](Panels-And-Window-Customization-Plan.md#b3-custom-menus--nativemenu-removed), which replaces native menus with a virtual `MenuBar` widget (built from the in-window `OverlayHost` `PopupMenu`/`ContextMenu`) consuming the existing `UI/Menu.h` data model. Because menus become ordinary virtual widgets composited through WTK's own Vulkan content — not GTK objects — there is no `GtkMenu`/`GtkMenuBar`/`GMenu` surface left for §2.15 to migrate, and the GTK-3 "menu-bar inset subtracted from event Y" coupling (§2.13 Risk 1) is retired with the native menu bar rather than revisited. (macOS keeps `NSApp.mainMenu` via the stand-alone `MacAppMenu` helper from §B3; that is a Cocoa-only concern and does not touch the GTK backend.) **Consequence for §2.15:** the `GtkMenu` row in the API-mapping table above and phase **G4-5** below are dropped; sequence the deletion of `GTKMenu.cpp` against whichever of the two plans lands first (if §2.15 reaches G4-5 before §B3 ships, simply leave `GTKMenu.cpp` on its GTK-3 path untouched and let §B3 delete it — never spend effort porting it to GTK 4).

#### Implementation phasing

Large subsystem (every GTK file touched) → full phasing. Seven reviewable steps; each compiles in isolation against the previous. GTK 3 stayed buildable behind the `WTK_GTK_MAJOR` flag through G4-1→G4-6; **G4-7 retired it (full clean removal — flag and all `#if WTK_GTK_MAJOR` guards deleted, gtk4 required outright)**, so the Linux backend is now GTK-4-only. ✅ **All phases complete.**

1. **G4-1 — Build: add `gtk4` pkg-config behind a flag. ✅ Done (configure-verified, both series).** `pkg_check_modules(GTK4 REQUIRED gtk4)` gated on a new `WTK_GTK_MAJOR=3|4` (default `3`). No source change; validated by the GTK 3 build still configuring + building unchanged.
   - **Landed (`wtk/CMakeLists.txt`, CMake-only):** new `WTK_GTK_MAJOR` cache `STRING` (default `3`, `STRINGS` `3 4`). The GTK pkg-config check branches on it — `gtk+-3.0` for `3`, `gtk4` for `4` — and the chosen series' output is normalized into series-agnostic `GTK_INCLUDE_DIRS` / `GTK_CFLAGS_OTHER` / `GTK_LIBRARY_DIRS` / `GTK_LIBRARIES`, so the eight downstream `OmegaWTK_Native` / `OmegaWTK` wiring references were retargeted off the hard-coded `GTK3_*` names (zero flag/link change for the `3` path — alias only). `add_compile_definitions(WTK_GTK_MAJOR=${WTK_GTK_MAJOR})` pushes the major to the compiler so the `#if WTK_GTK_MAJOR == 4` source branches in G4-2+ can switch on it (no such `#if` exists yet, so this is inert for now). An out-of-range value errors with a `FATAL_ERROR`. **Not touched:** `gte/tests/vulkan/CMakeLists.txt`'s independent `pkg_check_modules(GTK gtk+-3.0)` — it is a standalone Vulkan test harness outside §2.15's `wtk/src/Native/gtk/` scope; flagged for G4-7 cleanup.
   - **Verified:** default (`WTK_GTK_MAJOR=3`) — `cmake` exits 0, "GTK series = 3 (gtk+-3.0)", `ninja BasicAppTest` links clean (0 errors). `-DWTK_GTK_MAJOR=4` — `cmake` exits 0, "GTK series = 4 (gtk4)" (gtk4 4.22.4 found behind the flag); not built, since the source is still GTK 3 and G4-1 lands no port (a `4` build first compiles at G4-2+).
2. **G4-2 — App + window skeleton. In progress — resize mechanism proven; production port gated on the G4-2↔G4-3 fusion below.** Port `GTKApp.cpp` and `GTKAppWindow.cpp` window creation/lifecycle to GTK 4 under `#if WTK_GTK_MAJOR == 4`. **Per the Option B′ revision, `GTKAppWindow` no longer wraps a `GtkWindow`** — it owns a bare toplevel `GdkSurface` (`gdk_surface_new_toplevel`), sized via `compute-size` + the `layout` signal and mapped via `gdk_toplevel_present`. **`GTKApp.cpp` needs no GTK-4 rewrite** — `GtkApplication` / `g_application_run` / the `activate`/`open`/`shutdown` signals + the `g_application_hold` keep-alive are all GTK-4-valid, and a bare `GdkSurface` does not need to be a `GtkApplicationWindow`, so the existing app object drives the GTK-4 run loop and dispatches GDK events to the surface unchanged. No rendering yet — toplevel comes up as an empty owned surface.
   - **User edge-resize (developer requirement). Done in the reference (gte `2DTest`), build- + run-stable.** A bare decorationless toplevel gets no resize for free, so it is client-driven: on `GDK_MOTION_NOTIFY` hit-test an edge band (12 logical px) and set the matching CSS resize cursor (`ns-`/`ew-`/`nwse-`/`nesw-resize`) via `gdk_surface_set_cursor`; on a primary `GDK_BUTTON_PRESS` in a band, hand the drag to the compositor with `gdk_toplevel_begin_resize(toplevel, edge, device, button, x, y, time)` (on Wayland the compositor is the only thing that can resize). This is the mechanism `GTKAppWindow` will reuse (and is the seed of Part B1's `ClientDecorationRegions`).
   - **Finding — G4-2 fuses with the G4-3 surface-handle retarget.** Under Option B′ the root `NativeItem` (`GTKItem`) can no longer be `GtkWidget`-backed: it resolves the protocol handle today via `gtk_widget_get_window()` (GTK-3-only) and must instead wrap the bare `GdkSurface` (`gdk_x11_surface_get_xid` / `gdk_wayland_surface_get_wl_surface`) — which *is* the §2.14 `VKVisualTree` retarget the plan had scoped at G4-3/G4-7. So a runnable `GTKAppWindow` skeleton can't be produced without also moving `GTKItem` (and the realize handoff that feeds `SurfaceHost`) onto `GdkSurface`. Additionally, the **full GTK-4 WTK library does not link until G4-6** (Screen/Theme/Clipboard/Dialog still GTK 3), so the GTK-4 backend is being landed as a **coordinated G4-2→G4-6 sweep** (developer decision) with `./build` flipped to `WTK_GTK_MAJOR=4`; each file is object-compile-verified as it lands and the first full link/run comes at the end of G4-6 (the `2DTest` bare-surface reference stays the runtime proof until then).
   - **Structural trio ported — object-compile-verified under GTK 4 (0 errors / 0 warnings):**
     - `GTKItem` (`.h`/`.cpp`) — root now wraps a bare `GdkSurface` (no `GtkWidget`/`GskRenderer`); protocol-handle accessors source from the surface (`gdk_wayland_surface_get_wl_surface` / `gdk_x11_surface_get_xid`, X11 behind the GTK-4.18 deprecation guard); child-item / scroll / clipped-view machinery dropped (children are subsurfaces owned by the VisualTree via the §2.13a `SurfaceHost` registry). `getWidget()`→`getSurface()`.
     - `VKVisualTree.cpp` — resolves the root display/handle from the `GdkSurface` (`gdk_surface_get_display`) instead of `getWidget()`; stays root-only (Pass 1).
     - `GTKAppWindow` — GTK-3 impl gated `#if WTK_GTK_MAJOR == 3`; new **`GTKAppWindow4.cpp`** (`#if WTK_GTK_MAJOR == 4`) implements the full `NativeWindow` interface on a bare `gdk_surface_new_toplevel`: `compute-size`+`layout` sizing, `gdk_toplevel_present` map, edge-resize via `gdk_toplevel_begin_resize` + hover cursors, realize-gate (`onLayout`→feed `SurfaceHost`→drain `onFirstRealize`), `requestFrameFlush` g_idle coalescing, min/maximize/fullscreen via `GdkToplevelLayout`, title/cursor/close/order/key-window. `setOpacity` is stored-only (GTK 4 has no per-`GdkSurface` opacity); full `GdkEvent`→`NativeEvent` input translation is deferred to G4-4. `GTKApp.cpp` needed no change. **Remaining sweep:** `GTKScreen` / `GTKTheme` / `GTKClipboard` / `GTKDialog` GTK-4 API swaps + exclude `GTKMenu` (Panels §B3), then full link + run.
3. **G4-3 — Root surface via the bare `GdkSurface` (GTE unchanged). ✅ Done (run-verified, Wayland).** `GTKItem` wraps the bare toplevel `GdkSurface`; the realize handoff (`onLayout` → feed `SurfaceHost` → drain `onFirstRealize`) gives WTK the protocol handle and it creates the `VkSurfaceKHR`/swap chain as in §2.13 — no GTE change, no `GdkVulkanContext`. **Run-verified:** BasicAppTest under `WTK_GTK_MAJOR=4` comes up, the surface realizes (`nativeReady` 0→1), `vkCreateSwapchainKHR` succeeds (no `SURFACE_LOST`), `renderCompositeFrame` deposits 21 slices and presents — window stays up, zero VK errors / Gdk warnings / protocol errors. GSK coexistence is resolved by construction (Option B′, no `GskRenderer` on the surface).
4. **G4-4 — Input via the GdkSurface `event` signal. ✅ Done (run-verified, live Wayland session — motion/click/key/scroll routing + edge-resize confirmed by developer).** Full `GdkEvent` → `NativeEvent` translation lands in `GTKAppWindow4.cpp`'s `onEvent` trampoline, dispatching per type to member handlers that emit through the window's own `eventEmitter()` — the same sink Win32 / macOS deliver to:
   - **Motion** (`GDK_MOTION_NOTIFY` → `handleMotion`): emits `CursorMove`; still sets the edge-resize cursor first when over the resize band of a resizable surface (the windowing affordance from G4-2 is preserved, now also emitting the virtual move).
   - **Buttons** (`GDK_BUTTON_PRESS`/`RELEASE` → `handleButton`): primary→`LMouseDown`/`LMouseUp`, secondary→`RMouseDown`/`RMouseUp`. A primary press inside the resize band still begins a compositor-driven `gdk_toplevel_begin_resize` and is consumed (returns `TRUE`) before the virtual layer sees it; middle/extra buttons are unmapped (pass through).
   - **Keys** (`GDK_KEY_PRESS`/`RELEASE` → `handleKey`): `KeyDown`/`KeyUp` with `keyCodeFromGdk(keyval)` (GTK-4 copy of the GTK-3 keysym map — `GDK_KEY_*` are series-identical) + raw `gdk_keyval_to_unicode` on `key`. `isRepeat` stays false (GTK 4 exposes no public repeat flag; auto-repeat arrives as a press stream).
   - **Scroll** (`GDK_SCROLL` → `handleScroll`): smooth via `gdk_scroll_event_get_deltas` → `ScrollWheel` + directional `ScrollLeft/Right/Up/Down`; discrete directions mirror the GTK-3 `±1`/`×10` convention.
   - **Crossing** (`GDK_ENTER_NOTIFY`/`GDK_LEAVE_NOTIFY` → `handleCrossing`): `CursorEnter`/`CursorExit`.

   Modifiers read off `gdk_event_get_modifier_state` (`GDK_ALT_MASK`, the GTK-4 rename of `GDK_MOD1_MASK`). GTK 4 GdkEvents carry no root/screen coordinate (Wayland has no global space), so `screenPosition` mirrors the surface-local `position`; virtual hit-testing uses `position`. Emission routes through a local `emitEvent` that always constructs the `NativeEvent` (which owns and frees `params`), so the no-receiver path leaks nothing. This unblocks the GTK-4 key path that §2.3a Focus / §2.2 DPI hotkeys depend on.

   **Two GTK-4-specific runtime defects surfaced during the verification run and were fixed (both invisible until the surface ran live):**
   - **Interactive resize snapped back.** `begin_resize` correctly handed the drag to the compositor and `onLayout` reported the growing size, but `onLayout` only stored the rect — it never told `AppWindow` to rebuild the swap chain. On Wayland the window's real size *is* the committed buffer size, so WTK kept presenting old-size frames and the compositor reverted the window. Fix: `onLayout` now emits `WindowWillResize` (→ `AppWindowDelegate` → `dispatchResizeToHosts`, the swap-chain rebuild) on each size-changed layout, plus a 120ms-debounced `WindowHasFinishedResize` — mirroring the GTK-3 configure-event path. `onLayout` also adopts the negotiated size as the new `requestedW_/H_` so `onComputeSize` stops re-proposing the construction-time size. The initial layout is excluded (startup size flows through the normal bring-up path).
   - **Scroll never reached the ScrollView.** GTK 4 scroll (axis) events carry no position — `gdk_event_get_position` returns FALSE and leaves the coords NaN — so the wheel hit-tested at `(NaN,NaN)`, matched no view, and was dropped. (This system's wheel also delivers *discrete* `GDK_SCROLL_UP`/`DOWN`, not smooth; the discrete branch already emits a `ScrollWheel` of `(0,±10)`, which is what `AppWindow`/`WidgetTreeHost` route.) Fix: track the last pointer position from motion/button/crossing events (`lastPointerX_/Y_`) and fall back to it when the scroll event has none.

   **Latent (not blocking, on record):** `AppWindowDelegate::onRecieveEvent` (`wtk/src/UI/AppWindow.cpp`) routes only `ScrollWheel` to `dispatchInputEvent` — directional `ScrollUp/Down/Left/Right` are dropped. Harmless today because `handleScroll` always also emits `ScrollWheel`; worth closing when the scroll surface is generalized.
5. **G4-5 — Menus. ✅ Done (excluded, not ported) — superseded by [Panels-And-Window-Customization-Plan §B3](Panels-And-Window-Customization-Plan.md#b3-custom-menus--nativemenu-removed).** `GTKMenu.cpp`'s GtkMenu bridge is gated `#if WTK_GTK_MAJOR == 3`; under GTK 4 the six `GTK::make_gtk_menu*` factories are no-op stubs (return null) purely so `NativeMenu.cpp`'s `TARGET_GTK` dispatch links until §B3 deletes the whole interface. `UI/Menu.cpp`'s `Menu` methods gained `if(native)` guards (matching the existing `MenuItem` idiom) so a null native menu no-ops cleanly. No `GMenu`/`GtkPopoverMenuBar` port.
6. **G4-6 — Screen / Theme / Clipboard / Dialog. ✅ Done (compile-verified; lib links + BasicAppTest run-verified, Wayland).** `GTKScreen.cpp` → `gdk_display_get_monitors()` `GListModel` (no primary/workarea; DPI folded into monitor scale). `GTKTheme.cpp` → dark/light appearance (`GtkSettings`) + Pango typography only; color extraction is GTK-3-only (per scope decision — defaults stand on GTK 4). `GTKClipboard.cpp` → async `GdkClipboard` read wrapped in a nested `GMainContext` loop to honor the synchronous `NativeClipboard` contract (Risk 3 resolved — see below); file paths via `GDK_TYPE_FILE_LIST`. `GTKDialog.cpp` → async `GtkFileDialog` / `GtkAlertDialog`, resolving the `Async<>` promise in the GAsyncResult callback (shared-promise pattern mirroring Cocoa); `GtkAlertDialog` for the note dialog. All gated `#if WTK_GTK_MAJOR`, GTK-3 paths preserved.
7. **G4-7 — Retire GTK 3; verify. ✅ Done (full clean removal; Wayland native run-verified, X11 via Xwayland smoke-verified, native-X11-session developer-pending).** GTK 3 is fully retired — **full clean removal** (developer decision): the `WTK_GTK_MAJOR` flag, its CMake branch, and every `#if WTK_GTK_MAJOR` source guard are gone; `wtk/CMakeLists.txt` now requires `gtk4` outright (the `GTK_*` names kept, aliasing gtk4). The GTK-3 `GTKAppWindow.cpp` (1293 lines) was deleted and the GTK-4 `GTKAppWindow4.cpp` **renamed → `GTKAppWindow.cpp`**; the `==3` arms were stripped from `GTKScreen` / `GTKTheme` / `GTKClipboard` / `GTKDialog` and the `==3` GtkMenu bridge from `GTKMenu` (its `==4` no-op stubs kept, per Panels §B3). The `VKVisualTree` `GdkWindow`→`GdkSurface` retarget the plan scoped here had already landed in the G4-2→G4-6 sweep (`gtkItem->getSurface()`), and §2.13a's `SurfaceHost`/co-build apparatus is reused untouched (the binary still links both `libwayland-client` and `libX11`/`libxcb`). **Verification:** full tree builds clean GTK-4-only; `BasicAppTest` brought up on a **live Wayland session** (`nativeReady` 0→1, swapchain selected, 21 slices composited + `Present`, 0 real errors) and under **`GDK_BACKEND=x11` (Xwayland)** (X11 `SurfaceHost` path realizes + presents identically). A **native X11 session** (real X server, not Xwayland) run-verify is the one arm left for the developer, per the "mark unverified backends" rule.
   - **Follow-up (out of §2.15 scope):** `gte/tests/vulkan/CMakeLists.txt` still does `pkg_check_modules(GTK gtk+-3.0)` for its *other* windowing probes (only `2DTest` is on gtk4). Converting those tests / dropping the gtk+-3.0 dep is a gte-test-harness cleanup, separate from retiring the WTK backend — removing it now would break the unconverted probes.

#### Risks / open questions

1. **GSK coexistence — RESOLVED by Option B′ (was the central risk).** The original concern was that GTK 4's GSK renderer would clear or repaint the toplevel out from under WTK's swap chain. The probe confirmed it is worse than that on a `GtkWindow`: GSK's renderer takes the surface and `vkCreateSwapchainKHR` fails outright (`VK_ERROR_SURFACE_LOST_KHR`). Option B′ removes the risk by construction — a bare `gdk_surface_new_toplevel` has no `GskRenderer` bound, so there is no GSK content to coexist with. Option C (offscreen → `GdkTexture` import) remains the fallback only if a future compositor refuses a directly-owned bare surface. There is still no GTE device adoption to negotiate — WTK keeps owning the Vulkan device and surface.
2. **No `GdkVulkanContext` failure path needed.** Because WTK creates the `VkSurfaceKHR` itself (not `gdk_surface_create_vulkan_context`), the GTK-4 Vulkan-context creation failure mode does not apply. WTK's existing "no Vulkan WSI → refuse with a legible error" path (already present on the §2.13 Linux backend) is unchanged.
3. **Clipboard goes async-only — RESOLVED via nested-loop (G4-6).** GTK 4's `GdkClipboard` has no synchronous `wait_for_text`. Rather than break the §2.6 sync contract, the GTK-4 `getText`/`getFilePaths` fire the async read and iterate the default `GMainContext` until the GAsyncResult lands — exactly the path `NativeClipboard.h` already documents ("async backends pump a nested run-loop"). No interface change; `getTextAsync()` remains a possible future addition but is not needed for the contract. (Runtime verification deferred to a clipboard-consuming widget — TextInput.)
4. **Theme color extraction regresses.** `GtkStyleContext` color lookup is deprecated/removed-in-spirit in GTK 4; reliably reading the OS accent/background as RGBA is genuinely harder (CSS-driven). §2.5's GTK `queryCurrentTheme()` colors may degrade to a coarser approximation on GTK 4 until a CSS-probe approach is validated. Appearance (light/dark) is still cleanly queryable.
5. **Non-compositing WM smoke test is moot now (a plus).** §2.13 Risk 3 (drawing into an X11 Window under a non-compositing WM) disappears — GSK owns presentation. One fewer environment to special-case.
6. **HiDPI Wayland buffer scale — found + fixed.** The swap chain is sized in *physical* pixels (`VKVisualBinder::fillDescriptor` → `rect.w × scale`), and on GTK 3 GDK set the matching `wl_surface` `buffer_scale` on its `GtkWindow` surface. The bare GTK 4 `GdkSurface` we render into ourselves never got it, so a 1200-px buffer was shown at `buffer_scale=1` → the window came up `scale`× too large (full-screen on a 2× 4K monitor — developer-observed). **Fix:** `GTKAppWindow4::applyWaylandBufferScale()` calls `wl_surface_set_buffer_scale(surface, integerScale)` on layout (before the first WSI commit and on every scale change); the physical-sized (crisp) buffer is kept and now maps to the correct logical size. Build- + run-verified, and **developer-confirmed: window is the correct size and renders crisp** on a 4K@2× Wayland monitor. Fractional scale (non-integer) still routes through the deferred `wp_fractional_scale_v1` + `wp_viewporter` work (§2.2 / §2.9).

#### Feature proposal — cross-platform per-app user scale (Ctrl +/− zoom)

GNOME (and most desktops) scale apps individually; WTK should expose the same as a **cross-platform** user-zoom, independent of the monitor/DPI scale. Design:

- A per-`AppWindow` `userScale_` multiplier (default 1.0), combined with the monitor scale: `effectiveRenderScale = currentScreen().scaleFactor × userScale`. This reuses the existing scale-propagation machinery (`ViewRenderTarget::setRenderScale` + the full-tree repaint the §2.2 / UIView-Render-Redesign Phase F handler drives) — the same path a monitor DPI change already takes, so every `DrawOp` re-rasterizes crisply at the new effective scale.
- Hotkeys: **Ctrl + `=`/`+`** zoom in, **Ctrl + `-`** zoom out, **Ctrl + `0`** reset (clamped, e.g. 0.5×–3× in fixed steps). Handled at the `AppWindow`/`WidgetTreeHost` level so it is uniform across backends.
- **Dependency:** the keyboard path. On macOS/Win32 the key events already flow; on **GTK 4 this is blocked on G4-4** (full `GdkEvent`→`NativeEvent` translation — only close/edge-resize are wired today). The scale-application half is backend-agnostic and can land first.
- Small feature (a scale multiplier + 3 hotkeys + reuse of existing renderScale propagation) — does not need its own multi-phase breakdown; implement once G4-4 unblocks the GTK key path. On Wayland it composes with the `buffer_scale` fix above (integer part) / the deferred fractional-scale work (fractional part).

#### Dependencies

**§2.15 depends on:** §2.13/§2.13a — it *reuses* their surface-ownership model and `SurfaceHost`/co-build apparatus, re-sourcing the native handle from `GdkSurface`. It also retargets §2.14's Linux `VKVisualTree` impl (keeping the abstract API). It requires **no** GTE-side change — GTE keeps owning the Vulkan device/surface, the same stance as §2.13a.

**§2.15 enables:**
- The GTK-4-gated Wayland follow-ups already deferred in §2.2 (`wp_fractional_scale_v1`) and §2.9 (`wp_presentation_feedback` display-link, fractional scale) — they attach to the GDK 4 `GdkSurface` WTK already resolves.
- §2.13a's "one binary, both sessions" outcome on a current GTK toolkit: the co-build apparatus runs unchanged on GTK 4, off a `GdkSurface` handle.
...
| ~~P1~~ **Source-complete (GTK compile-verified; macOS/Win32 off-platform)** | 2.6 NativeClipboard | Copy/paste is fundamental UX. Header + all three backends landed; runtime verification deferred to a clipboard-consuming widget. |
...
| **P2** | 2.15 GTK4 Migration | Linux backend off EOL GTK 3 onto GTK 4, adopted for **windowing + input only**. WTK keeps owning the Vulkan device/surface/swap chain (no `GdkVulkanContext`, no GTE device adoption); the native handle is sourced from `gtk_native_get_surface()` → `GdkSurface`. Preserves §2.13's surface model and reuses §2.13a's co-build apparatus. The work is GTK-3→GTK-4 API churn (window/input/clipboard/dialog), not a renderer rewrite. Menus are excluded — `NativeMenu`/`GTKMenu` are deleted by Panels-And-Window-Customization-Plan §B3 (virtual `MenuBar`), not ported. Gates the deferred Wayland fractional-scale / presentation-feedback follow-ups in §2.2 / §2.9. |
