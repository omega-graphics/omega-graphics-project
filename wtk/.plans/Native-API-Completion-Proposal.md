...
| 6    | `setTabOrder(a, b)` override                                                                       | hand-tuned form order               | ❌ Not started |

Steps 1–3 unblock TextInput. Steps 4–6 are independent and can land in any order. **Also landed:** M1 (mouse-click focus — `LMouseDown` focuses the nearest `isClickFocusable()` ancestor with `FocusReason::Mouse`), which is not a numbered step but gates the click-focus path. Remaining in the Focus block: Step 6 (`setTabOrder`).
