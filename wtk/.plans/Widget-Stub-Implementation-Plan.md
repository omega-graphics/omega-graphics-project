...
**Grid — [Implemented 2026-07-12].** Shipped as a `GridLayout : public LayoutManager` (`LayoutManager.h`/`.cpp`) driven by a `Grid : public Container` (`Containers.h`/`.cpp`), following the 4.5 precedent (`StackWidget` ↔ `FlexLayout`; `Grid` ↔ `GridLayout`) — a container kind is a `LayoutManager` set on the backing View, never a bespoke `layoutChildren()`. `GridLayout` places children row-major (first-fit into an occupancy grid) across equal-width columns, honoring `columnSpan` / `rowSpan`; column widths are uniform, row heights are content-driven (each row sizes to the tallest single-row child that lands in it, and a taller multi-row spanner grows the last row it covers). `GridCellAlign` (Start/Center/End keep the child's size and position it; Stretch fills the cell block) maps from the widget's `GridOptions.cellAlign` (a `StackCrossAlign`), exactly as `StackWidget` maps `StackCrossAlign` → `FlexCrossAlign`. `Grid` mirrors `StackWidget`'s drive path: own the layout, `view->setLayoutManager(&gridLayout_)` in the ctor, `relayout()` on mount / resize / child change, per-child span spec wired on `addChild`. `minSize` is not overridden (base default = current rect) — a future revision can compute the true grid intrinsic. Verified in `BasicAppTest` (a 3-column grid with a 2-column span). **Deferred:** per-column proportional/auto widths (uniform only), row/column intrinsic `minSize` aggregation.
...
**ContextMenu — [Implemented 2026-07-12].** Shipped in `wtk/include/omegaWTK/Widgets/Overlays.h` + `wtk/src/Widgets/Overlays.cpp`, presenting through the Overlay-Z-Order-Plan infrastructure (`OverlayTier::Floating`). Structure: a `Widget` whose root `UIView` is the menu background, with one `UIView` **sub-view per item** (`makeSubView`) carrying its own `bg` / `label` / `shortcut` elements — sub-views (not single-view + hit-math) so per-item mouse hover works through the standard `onMouseEnter`/`onMouseExit` the WidgetTreeHost hover synthesis already fires, and per-item click through `onLeftMouseUp` with no position arithmetic. Two private `ViewDelegate`s: an `ItemDelegate` per interactive row (hover + click), and one `KeyDelegate` on the root for `ArrowUp`/`ArrowDown`/`Enter` (Escape and Tab never reach it — the host intercepts them first; Escape rides O3's `dismissTopmostForEscape`). Focus: the root claims `StrongFocus` + `focus(Popup)` on present so keys route to it, and O4 returns focus to the opener on dismiss automatically (Floating tier participates in restoration). Reaching the host: `ContextMenu::present(anchor, windowPos)` reads `anchor->treeHost->overlayHost()` via a targeted `friend class ContextMenu` on `Widget` (the anchor is a base `Widget*`, so protected-through-base access does not apply); the menu derives `std::enable_shared_from_this<ContextMenu>` to hand itself to `OverlayHost::present` (which then owns it while shown). Verified end-to-end in `BasicAppTest` (an "Open Context Menu" button; smoke-tested launch, screenshot verification of the presented menu + navigation is the visible check). **Deferred:** nested `submenu` (the `PopupMenuItem::submenu` field is modeled but not built), the `AtWidget`-edge anchor mode (context menus open `AtPoint` at the cursor), and a distinct `PopupMenu` variant (same recipe).

**Four overlay-infrastructure bugs the first live ContextMenu exposed (fixed 2026-07-12).** All four are general overlay infrastructure — a future Popover / Modal / Snackbar benefits from every one, they were not ContextMenu-specific patches. The first two blocked rendering / input; the last two were use-after-frees found by driving the menu (the second via a gdb backtrace after guessing missed).

1. **Overlay sub-views rendered blank.** `FrameBuilder::buildFrame`'s Style and Layout passes prune any subtree whose own `(dirtyBits | descendantDirty) & passBit == 0`, but `OverlayHost::present` marked only the overlay *root* dirty. The main tree gets a whole-subtree mark for free (`initWidgetTree` / `forceFullRepaint`); an overlay does not, so a ContextMenu's item sub-views were visited by the Paint pass (which walks the whole tree) yet skipped by Style/Layout → they painted with unresolved styles and unlaid-out elements = invisible. Fixed in `WidgetTreeHost::mountOverlay` by `markFullRepaintRecurse(*overlay->view)` — marks the entire overlay subtree.

2. **Hover / click never reached the item rows.** `WidgetTreeHost::hitTestOverlay` reused `hitTestWidget`, which descends the **widget tree** (`childWidgets()`). A ContextMenu's items are raw `UIView` sub-views (`makeSubView`), not child Widgets, so the descent found nothing and always returned the overlay root — the per-item hover/click delegates never fired. Fixed by walking the **view tree** in the overlay hit-test (`hitTestOverlayViewSubtree`): it reaches raw sub-views, and since a child Widget's root view is itself a sub-view of its parent, it also covers the child-Widget case for free.

3. **Use-after-free dismissing from a click.** An item's click ran `ItemDelegate::onLeftMouseUp` → `dismiss()`, which destroyed the menu *synchronously* while the dispatcher + that delegate were still on the stack — freeing the `View`/`ViewDelegate` being dispatched (a scoped `keepAlive` dropped as soon as the handler returned, too early). Fixed with a deferred-teardown ("delete-soon") path: `OverlayHost::requestDeferredDismiss` queues the handle, and `WidgetTreeHost::dispatchInputEvent` drains the queue (`drainDeferredDismissals`) from a scope-guard that fires at every exit, i.e. after the whole event has unwound. `ContextMenu::dismiss` now routes through it. General to any overlay that closes itself from its own input handler.

4. **Use-after-free on the next mouse-move after dismiss.** The dispatcher caches `hoveredView_` (and `capturedView_`) across events. Once the menu was torn down, `hoveredView_` still pointed at a freed item sub-view, and the next motion event dereferenced it (`hoveredView_->setPseudoClassBits(...)`). Fixed in `WidgetTreeHost::unmountOverlay` (runs during teardown, views still alive): if either cached pointer is inside the dismissed overlay's subtree (`viewIsInSubtree`), drop the stale hover pseudo-class and null it. Sibling of the FocusManager dangling-focus case O4 already guards.

**Future migration — [not scheduled].** Rework `ContextMenu` to subclass `Container` and render each row as a `Button` child (per user direction 2026-07-12). Rationale: `Button` already ships robust hover/press state, transitions, disabled styling, and `onPress`, and `Container` owns child layout, so the menu gets those behaviors and more per-row customization for free instead of hand-rolling per-item delegates + restyle. **Blocker to resolve first:** overlay-hosted `Container` children need their `onMount()` to run so `Button::rebuildContent` builds its elements — the main tree gets this via `WidgetTreeHost::initWidgetRecurse` (`parent->init()` → `onMount()`), but `mountOverlay` does not currently init the overlay subtree. The migration must extend `mountOverlay` to init-walk the presented subtree (mirroring `initWidgetRecurse`), or `present` must run it, before child Buttons will paint. Keyboard nav (Up/Down/Enter) then moves a highlighted index and drives the corresponding `Button`'s visual/press; the current bespoke sub-view + `ItemDelegate`/`KeyDelegate` design is the interim until then.
...
| **Phase 4B TextInput base** (DONE)                                                                    | [Native-API-Completion-Proposal §2.3a Focus](Native-API-Completion-Proposal.md#focus--virtual-focus-manager) **steps 1–3** (`FocusPolicy`, `FocusManager` skeleton, key routing)    | TextInput must claim keyboard focus and receive `KeyDown` from `WidgetTreeHost`. Without FocusManager the routing is undefined.                                                            |
...
| **Phase 4B accurate caret X** follow-up                                                                    | This plan's own **Phase 2A Text Measurement** (`UIView::measureText`)                                                                                                     | Caret position must align with glyph advances.                                                                                                                                            |
