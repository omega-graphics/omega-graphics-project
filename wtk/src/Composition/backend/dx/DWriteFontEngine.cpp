...
/// @note The Collection Key is the font file path to be loaded.
    ...
    public:
        ...
        // Phase 7 stub. The MSDF path is the production target on
        // Windows; the bitmap fallback path used to live in
        // `DWriteTextRect` (retired with the rest of the per-engine
        // layout code). When a Windows font genuinely falls back to
        // BitmapFallback mode (color emoji, bitmap-only faces) the
        // glyphs disappear instead of rendering. Reimplement via D2D
        // `ID2D1RenderTarget::DrawGlyphRun` when a real Windows
        // user needs color emoji rendering.
        BitmapTextResult rasterizeSubRunToTexture(
            ...
            // Phase 7: fallback for the WTK layout-engine path goes
            // through `DWriteFontFallback::fallbackForCodepoint` —
            // which calls `CreateFont` to materialize a fresh
            // `DWriteFont` with its own text format + face, mirroring
            // the macOS Core Text backend.
