...
void Compositor::registerWindowSurface(SharedHandle<CompositionRenderTarget> target,
    ...
    // Wire the deposit-driven frame trigger before publishing into the
    // map so the scheduler can never observe a registered surface
    // without a wake path.
    ...
    // A surface registered after content already exists (e.g. an
    // initial paint that ran before AppWindow finished wiring) should
    // get rendered on the next loop iteration rather than wait for the
    // next deposit.
    notifyFrameDirty();
}
...
void Compositor::drainWindowSurfaces(){
    // Snapshot under the lock so deposits arriving mid-drain don't
    // mutate the map while we iterate. Surfaces themselves are
    // refcounted and safe to render against without the compositor lock.
...
void Compositor::renderCompositeFrame(const SharedHandle<CompositionRenderTarget> & target,
    ...
    // 1. Resolve / create the BackendCompRenderTarget. First-frame
    //    bootstrap path mirrors the old executeCurrentCommand(Render)
    //    branch: pull pre-created visual tree resources out of the
    //    PreCreatedResourceRegistry that View construction populated.
    ...
    // 2. Ensure the root visual exists. Per Tier 0, the root visual IS
    //    the render context — every slice draws into it via viewport
    //    overrides; there are no per-layer surfaces.
    ...
    // 3. Pick a clear color from the first slice that actually wants a
    //    background. Subsequent slices paint over the cleared frame.
    float clearR = 0.f;
    float clearG = 0.f;
    float clearB = 0.f;
    float clearA = 0.f;
    ...
    // 4. One render pass for every slice in the composite frame.
    ...
    // 5. Effects run as their own passes today. Tier 2 Phase 3 reduces
    //    this; for now apply per-slice effects after the batched pass so
    //    they read the just-rendered content.
    for(auto & slice : frame->slices){
    ...
    // 6. One commit + present. Phase A-1 makes commit() present
    //    directly to the native drawable.
