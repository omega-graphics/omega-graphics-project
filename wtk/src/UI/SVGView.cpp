...
Composition::Path parseSVGPathData(const OmegaCommon::String & d) {
    ...
    auto ensureStarted = [&](float x, float y) {
        if (!started) {
            path.goTo(OmegaGTE::GPoint2D{x, y});
            startX = x;
            startY = y;
            curX = x;
            curY = y;
    ...
    while (tok.hasMore()) {
        ...
        case 'M':
            ...
            if (started) path.goTo(OmegaGTE::GPoint2D{x, y});
            ensureStarted(x, y);
            curX = x; curY = y;
            startX = x; startY = y;
            cmd = 'L';
        ...
        case 'L':
            if (!tok.readNumber(x) || !tok.readNumber(y)) goto done;
            ensureStarted(x, y);
            path.addLine(OmegaGTE::GPoint2D{x, y});
            curX = x; curY = y;
            break;
        case 'l':
            if (!tok.readNumber(x) || !tok.readNumber(y)) goto done;
        ...
        case 'H':
            ...
            path.addLine(OmegaGTE::GPoint2D{x, curY});
            curX = x;
            break;
        case 'h':
            if (!tok.readNumber(x)) goto done;
        ...
        case 'V':
            ...
            path.addLine(OmegaGTE::GPoint2D{curX, y});
            curY = y;
            break;
        case 'v':
            if (!tok.readNumber(y)) goto done;
        ...
        case 'C': {
            ...
            path.addLine(OmegaGTE::GPoint2D{x3, y3});
            curX = x3; curY = y3;
            break;
        }
        case 'c': {
            ...
            path.addLine(OmegaGTE::GPoint2D{ex, ey});
            curX = ex; curY = ey;
            break;
        }
        case 'Z':
...
OmegaCommon::Vector<OmegaGTE::GPoint2D> parsePointsList(const OmegaCommon::String & pts) {
    ...
    while (p < end) {
        ...
        if (after == p) break;
        p = after;
        result.push_back(OmegaGTE::GPoint2D{x, y});
    }
    return result;
}
...
void walkElement(Core::XMLDocument::Tag & tag, OmegaCommon::Vector<SVGDrawOp> & ops) {
    auto name = tag.name();
    if (!tag.isElement())
        return;
    ...
    if (name == "rect") {
        ...
        if (rx > 0.f || ry > 0.f) {
            ...
            op.roundedRectGeom = Core::RoundedRect{Core::Position{x, y}, w, h, rx, ry};
        } else {
            op.type = SVGDrawOp::Type::Rect;
            op.rectGeom = Core::Rect{Core::Position{x, y}, w, h};
        }
        ...
        op.ellipseGeom = Core::Ellipse(cx, cy, r, r);
        parseStyleAttrs(tag, op);
        ops.push_back(std::move(op));
    }
        ...
        float ry = parseFloatAttr(tag, "ry");
        op.type = SVGDrawOp::Type::Ellipse;
        op.ellipseGeom = Core::Ellipse(cx, cy, rx, ry);
        parseStyleAttrs(tag, op);
        ops.push_back(std::move(op));
    }
    else if (name == "line") {
        ...
        Composition::Path p(OmegaGTE::GPoint2D{x1, y1});
        p.addLine(OmegaGTE::GPoint2D{x2, y2});
...
void SVGView::rebuildDisplayList() {
    ...
    for (auto & child : children)
        walkElement(child, drawOps_->ops);
    needsRebuild_ = false;
}
