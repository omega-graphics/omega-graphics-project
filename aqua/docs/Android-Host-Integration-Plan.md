...
Status: **Plan** — not implemented. AQUA on Android currently ships a
compile-only stub at `aqua/src/platform/android/AndroidWindow.cpp` and
`aqua/target/android/main.cpp` so the cross-compile toolchain pipeline can
...
in `aqua/src/platform/ios/UIKitWindow.mm` and `aqua/target/ios/main.mm`.
...
AQUA's public API on desktop is:
...
4. **Library, not executable.** Android apps are APKs containing one or
   more `.so` files plus a Java/Kotlin shell. The AQUA build already
...
+---------------------------------------------------------------+
       |                          |                       |
       v                          v                       v
  Aqua::CreateApp()       AppCmd::INIT_WINDOW        AppCmd::TERM_WINDOW
       |                  -> Window::create           -> destroy native RT
       |                  -> rebuild GTE native RT
       v
  Aqua::App ctor (+ onInit() once the surface exists)
       |
       v
  per-loop iteration:
...
1. In `aqua/CMakeLists.txt` under the Android branch, add the
   `android_native_app_glue.c` source from the NDK to AQUA's compile.
...
3. In `aqua/cmake/AquaGame.cmake` Android branch, ensure the produced
...
void HandleCmd(android_app *app, int32_t cmd) {
    ...
    switch (cmd) {
        case APP_CMD_INIT_WINDOW:
            // Surface exists. Construct the AQUA App now (its Window
...
- **App construction timing.** Don't call `Aqua::CreateApp()` until the
  first `APP_CMD_INIT_WINDOW`. The AQUA `App` ctor calls
...
Rewrite `aqua/src/platform/android/AndroidWindow.cpp`:
...
std::unique_ptr<Window> Window::create(const WindowDesc &desc) {
    ...
    ANativeWindow *native = AquaAndroid::GetActiveNativeWindow();
...
The `AquaAndroid::GetActiveNativeWindow()` accessor is declared in a small
internal header (`aqua/src/platform/android/AndroidNativeWindowAccess.h`)
...
`HandleInput` translates `AInputEvent` → AQUA's input model. AQUA does
...
The current `aqua/cmake/AquaGame.cmake` Android branch produces
...
- A minimal Gradle/AGP project that wraps the `.so` into an APK. Most
  practical option: ship a `aqua/target/android/gradle-template/` tree
...
Single proposed addition to `aqua/include/aqua/App.h`:
...
4. **`BasicGame` on Android**: once Phase 5 lands, drop the
   ...
   `aqua/CMakeLists.txt` so `BasicGame` builds and runs on Android too.
...
5. **`android_native_app_glue` is C** — fine to compile into AQUA, but
   if AQUA grows another C compile-unit we should keep this in mind for
...
`aqua/docs/done/` (mirroring the existing pattern in this directory) and
update `aqua/CMakeLists.txt` to drop the Android-specific BasicGame skip.
