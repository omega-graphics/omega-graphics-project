...
### 14.3 Increments (thin-slice-first) — ALL FOUR DONE (implemented + verified 2026-07-07)

- **6f — Integer-death unification. DONE.** `AQdeathCountdown(sampledLifetime, substepDt)` lives in the SHARED `AQParticleMath.h` (production, oracle, and — by integer decrement — the GPU age kernel all derive the identical schedule): `ceil(lifeD / substepDt)` in double, non-positive → 1 (dies first age), saturating at uint32 max. `age()` decrements the integer and kills at zero; the float `lifetime` is aged for READBACK DISPLAY only, never consulted for death. `emit`/`particlesEmit` gained the `substepDt` parameter (AQContext passes `fixedDt`); countdown is frozen at emission, so a mid-flight `setFixedTimestep` rescales remaining wall-clock life (documented edge). Oracle retrofitted; full particle suite re-verified (census exact, trajectory band unchanged at 2.4e-5).

- **6g — Reusable GPU scan primitive. DONE — as reduce-then-scan, not the Hillis–Steele seed.** The 2026-07-07 GPU-port recency sweep (recorded in Phase 7 §13.5-B, Q3) upgraded the plan: portable practice ships MULTI-PASS reduce-then-scan (FidelityFX/GPUSorting), and the single-pass alternative (decoupled look-back) requires forward-progress guarantees that can deadlock on Metal hardware — a hard no-go. `src/kernels/AQScan.omegasl`: `AQScanBlockExclusive` (128-lane threadgroup-shared-memory block scan; host dispatches FULL blocks so tail-block lanes exist on every backend; exclusive-from-inclusive; lane 127 emits the block total) + `AQScanAddOffsets`; `AQComputeBackend::encodeScanExclusive` recurses over the block sums (depth ≤ 3 to 2M elements) into any caller-supplied uint buffers, with `scanExclusive` as the standalone host surface. O(n) global traffic vs the narrowphase lambda's O(n log n). Test `aqua_gpu_scan_test`: bit-exact vs a CPU prefix sum at every block/level boundary (1…300k, incl. the 0/1 alive-flag shape), byte-identical re-run.

- **6h — Live GPU path. DONE (collision included via 6i, same kernel file).** `src/kernels/AQParticles.omegasl`: `AQParticleInject` (host-staged new particles → the pool tail — GEBufferWriter cannot offset-write, so emission stages into Upload buffers and a kernel appends), `AQParticleIntegrate` (per-lane five-branch field eval transliterated from `AQevalField<float>` + semi-implicit Euler), `AQParticleAge` (integer countdown — bit-exact with the host schedule), `AQParticleScatter` + `AQParticleRebuildFlags` (stable compaction at the 6g scan's offsets into a ping-pong destination; flags rebuilt from the HOST-predicted census — §14.2's no-readback claim made real). Backend: per-system resident pools keyed by the opaque system id (`ParticlePoolGPU`; Readback residency, Upload staging), `particlesEnsurePool/UploadStaging/UploadFields/UploadColliders/EncodeFrame/DownloadState/ReleasePool`; ONE command buffer + one sync per advance-frame (inject → nSub × (integrate [+ collide] + age) → scan → scatter → flag rebuild). Space side: on the GPU path the per-sub-step host work shrinks to `particlesAgeSubstep` (integer bookkeeping only); `particlesGpuFrame` stages the emission slice `[emitBase, emitBase+k)` (with a pre-age countdown copy — the host's age() has already decremented by encode time) and encodes; the host compaction then mirrors the same permutation from the same integer schedule. Readback sync points (`readParticleState`, debug drain, CPU-path re-base after a path switch) download the packed prefix lazily via a `gpuDirty` flag. `AQContext::loadKernels(path)` (public) loads `AQKernels.omegasllib` + runs the capability probe and flips `kernelsLive()`; `AQExecPath::GPU`/`Auto` then resolve GPU. Spaces hold a non-owning backend pointer severed by `~AQContext` (`detachCompute`).

- **6i — GPU collision + full fountain. DONE.** `AQParticleCollide` transliterates `AQshapeSignedDistanceGeneric<float>`'s closed forms (sphere/box/capsule/plane; hulls filtered at upload — +inf on the CPU path too) including the exact `AQTransform::inverse().transformPoint` operation order; colliders upload per advance as COMBINED world transforms with host-pre-normalized plane normals; push-out + into-surface velocity reflection on-device.
  - *Verified (`aqua_gpu_particle_test`, PUBLIC surface, device-guarded):* collision-free fountain vs the double oracle — census EXACT every frame, max pos gap **2.4e-05** (identical to the CPU path's 6e band), peak 5.8k live; full floor+box+sphere fountain — census exact, worst penetration **0.0** (never inside), pos gap vs oracle 1.2e-04 within the contact band; two independent GPU runs byte-identical; **135k live** at scale with zero per-frame readback (the census is host-side by construction). Full suite 23/23 green.

### 14.4 Risks & notes
- **SDF-in-OmegaSL (6i)** is the largest authoring risk — quaternion-conjugate/rotate + per-shape branches in the shader. The `AQParticleCollision.h` template is the exact spec to transcribe; a small stage test can compare the kernel's distance/normal to `AQshapeSignedDistanceGeneric<float>` before wiring into the pipeline. *(Outcome: the direct transliteration passed the full-fountain parity + no-tunnel invariant on the first run — the dedicated distance/normal stage test proved unnecessary.)*
- **Determinism stance holds:** GPU-float is *not* bitwise-equal to CPU-float (field-sum rounding order differs) — that is expected and allowed (*stable-cross-path, bitwise-within-path*). Both must match the oracle census-exact + trajectory-band; the emission count (double, host) and death (integer, host) are shared, so only trajectories carry a band. *(Measured: both paths land the SAME 2.4e-05 band vs the double oracle.)*
...
- **`AQParticlePool` (public POD view)** — noted for the kREATE handoff rather than wired: the 6h readback path serves `readParticleState` via the lazy prefix download; a zero-copy/mapped view is a follow-on once kREATE's consumption pattern exists.
- **GPU-port recency sweep (2026-07-07, full record in Phase 7 §13.5-B):** Q3 upgraded 6g to reduce-then-scan (decoupled look-back deadlocks without forward-progress guarantees — no-go on Metal); Q4 confirmed scan+stable-scatter as the only order-preserving compaction family; Q6 confirmed host-seeded emission + integer death as the right determinism trade against the (documented-nondeterministic) atomic-append mainstream, and flagged **counter-based/stateless RNG (Philox/Threefry)** as the future unlock for GPU-side emission if the host-emit upload ever bottlenecks.
- **Error posture on the live path:** a failed pool creation / upload / encode is LOUD (stderr names the system) and skips that system's frame — the device state visibly freezes rather than silently diverging from the host census.

---

*Brief status: **CPU-first pillar IMPLEMENTED (6a–6e, §13.3); GPU sub-phase IMPLEMENTED AND LIVE (6f–6i, §14.3, verified 2026-07-07)** — AQUA's first live GPU pillar, selectable via `AQContext::loadKernels` + `AQExecPath::GPU` with the CPU path as the at-parity fallback, on the shared timestep (§14.1), with the reusable reduce-then-scan primitive (6g, algorithm upgraded by the Phase-7 §13.5-B recency sweep) and the host-bookkeeping / device-float split of §14.2 delivered as designed. New files: `src/kernels/AQScan.omegasl`, `src/kernels/AQParticles.omegasl`, `src/AQComputeParticles.cpp`, `src/AQComputeUtil.h`; tests `aqua_gpu_scan_test.cpp`, `aqua_gpu_particle_test.cpp`. The proposal was accepted with corrections; the implementation contract is §13 (CPU) + §14 (GPU) — where §1–§12 and §13/§14 disagree, §13/§14 govern. Phase 6 ships **no** neighbor grid (deferred to Phase 7's successors: self-collision/fluids).*
