...
*Brief status: proposal. The open decisions in §11 (pool strategy, grid-vs-compact-hash split, one-way coupling, determinism enforcement, force-field count) should be settled before implementation begins. This brief follows the conventions of the Phase 2 and Phase 5 briefs — AQ-prefixed POD types, compute-first with a CPU fallback at parity, a `double` reference oracle, and additive debug-bus flag bits — and reuses their shipped substrate (grid, scan, radix sort, buffer pool) rather than redefining it.*
