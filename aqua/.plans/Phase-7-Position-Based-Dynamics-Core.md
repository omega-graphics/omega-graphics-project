...
*Brief status: **CPU-live core IMPLEMENTED (7a–7e, §13.3); GPU sub-phase IMPLEMENTED AND LIVE (7f, §13.4 — stage parity + the live flip, selectable via `AQContext::loadKernels` + `AQExecPath::GPU`, verified 2026-07-07); coupling / upgrade sub-phases PLANNED (7g–7i).** §11 #1 is settled: UNIFIED destination (developer), vet-corrected path (§13.0/§13.5-A) — rigid stays impulse near-term, AVBD is the oracle-gated unification target (7i trigger sharpened in §13.5-B: Newton de-experimentalizing SolverVBD), and the interface in §7 is frozen in its §13.1-corrected form. The GPU-port recency sweep (§13.5-B) confirmed every port choice; its determinism findings supersede 7h #3's phrasing (gather-based Jacobi, not scatter-atomics, is the deterministic evolution path). Where §1–§12 and §13 disagree, §13 governs.*
