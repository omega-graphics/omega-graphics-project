...
- **7f — GPU sub-phase.** OmegaSL kernels `predict` / `project(color batch)` / `derive` dispatched through `AQComputeBackend` — one dispatch per color, buffers resident across slices, constraint arrays uploaded on topology change only. Stage-isolation parity first (the 5c–5e pattern), live-path flip after Phase 6's 6f–6i establishes the live-GPU wiring conventions. The CPU loop already visits constraints in the exact order the colored dispatches will.
- **7g — Rigid↔XPBD contact coupling (the seam Phases 8/9 need).** Two-way coupling at the contact level under the one clock: XPBD particles read rigid collider poses (the Phase 6 one-way push-out generalized to constraint form), rigid bodies receive the reaction impulses back. This work is required under EITHER fork and is the near-term "unified" (§13.0): one substep loop, coupled at contacts — the Newton/PhysX shape.
- **7h — Convergence + robustness upgrades (flagged from the 7e data and the sweep).** (1) Long-chain/stiff convergence beyond raw substeps: multi-layer XPBD (Mercier-Aubin 2024) and/or long-range attachments for pinned ropes/cloth. (2) Far-from-origin hardening: the derive step's `(x − x_prev)/h` loses digits at large world coordinates (the solver2d XPBD precision bug); solve on position deltas / origin-relative when kREATE's world size demands it. (3) VBD vertex coloring: colors *vertices* instead of constraints (far fewer colors → better GPU occupancy than Vivace-style constraint coloring) — evaluate for the 7f kernel port.
- **7i — AVBD prototype behind the interface, oracle-gated (the §13.0 unification target).** At the pre-Phase-8 re-audit: prototype an augmented-Lagrangian vertex-block pass (AVBD, SIGGRAPH 2025) behind the same typed-constraint + batch metadata; validate rigid stacking (incl. a high-mass-ratio case for the solver2d failure mode), friction, joints, and within-path determinism against the Phase 3/4 battery on this substrate. Migrate rigid capabilities only on equal-or-better quality AND performance; rigid-XPBD (Müller 2020, pre-seeded row compliance) stays the documented fallback recast.
...
*Brief status: **CPU-live core IMPLEMENTED (7a–7e, §13.3); GPU / coupling / upgrade sub-phases PLANNED (7f–7i, §13.4).** §11 #1 is settled: UNIFIED destination (developer), vet-corrected path (§13.0/§13.5-A) — rigid stays impulse near-term, AVBD is the oracle-gated unification target, and the interface in §7 is frozen in its §13.1-corrected form. Where §1–§12 and §13 disagree, §13 governs.*
