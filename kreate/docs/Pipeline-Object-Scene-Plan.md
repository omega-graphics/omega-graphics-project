# KREATE Pipeline, Object, Scene — Implementation Plan (GTE Isolation)

## Context

BasicGame currently works with raw GTE command buffers, render pass descriptors, and pipeline state objects. The goal is to add abstractions so users never touch GTE directly — **and so GTE headers never appear in KREATE's public API**. This keeps translation units small as KREATE grows.

**Architecture:**

```
User code (BasicGame)
  |  only includes kreate/ headers
...
| File | Purpose |
|------|---------|
| `kreate/include/kreate/Math.h` | KREATE math types (Vec3, Vec4, Mat4, Color) — no GTE includes |
| `kreate/include/kreate/Pipeline.h` | Pipeline public API (opaque handle, PipelineDesc with KREATE-only types) |
| `kreate/include/kreate/Object.h` | Object public API (transform, visibility, name) |
| `kreate/include/kreate/Scene.h` | Scene public API (hierarchy, camera, clear color, render) |
| `kreate/src/pipeline/Pipeline.cpp` | Pipeline impl — includes GTE, compiles shaders, creates pipeline state |
| `kreate/src/renderer/Renderer.h` | Internal renderer class (not public) — owns command encoding |
| `kreate/src/renderer/Renderer.cpp` | Renderer impl — includes GTE, manages render passes |
| `kreate/src/Object.cpp` | Object impl (getters/setters, pure data) |
| `kreate/src/Scene.cpp` | Scene impl (hierarchy, transform propagation, delegates to Renderer) |
| `kreate/src/Math.cpp` | Mat4 utilities (perspective, lookAt, translation, etc.) |

## Modified Files

| File | Change |
|------|--------|
| `kreate/include/kreate/App.h` | Remove `#include <OmegaGTE.h>`, remove `gte()` and `renderTarget()` accessors, add `createPipeline()` / `createPipelineFromLibrary()` factories |
| `kreate/src/App.cpp` | Add pipeline factory methods that forward to Pipeline::create with internal GTE ref |
| `kreate/CMakeLists.txt` | Add new source files to SOURCES |
| `kreate/tests/BasicGame.cpp` | Rewrite to use only KREATE types (no GTE includes) |

---

## 1. Math Types

**Header:** `kreate/include/kreate/Math.h`

KREATE-owned types so public headers never reference `FMatrix`/`FVec` from GTE.
...
**Header:** `kreate/include/kreate/Pipeline.h`
...
5. Map KREATE `CullMode`/`FillMode` -> GTE `RasterCullMode`/`TriangleFillMode`
...
**Header:** `kreate/src/renderer/Renderer.h` (NOT in `include/` — internal only)
...
**Header:** `kreate/include/kreate/Object.h`
...
**Header:** `kreate/include/kreate/Scene.h`
...
Uses only KREATE types — no OmegaGTE includes.
...
3. **Pipeline** — needs GTE internally, public header uses only KREATE types
...
7. **BasicGame.cpp** — rewrite to use only KREATE types
...
| `Pipeline.h` | None | New — uses KREATE enums |
| `Object.h` | None | New — uses `Kreate::Mat4` |
| `Scene.h` | None | New — uses `Kreate::Mat4`, `Kreate::Color` |
...
2. **Header isolation:** `BasicGame.cpp` compiles with zero GTE includes — verify no transitive GTE headers leak through KREATE public headers
