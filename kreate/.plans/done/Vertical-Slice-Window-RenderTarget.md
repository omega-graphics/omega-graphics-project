...
First vertical slice for KREATE — open a native window and render into it via OmegaGTE.
...
│    Instantiate Kreate::App, open window, run loop  │
└──────────────────┬───────────────────────────────┘
                   │ links
┌──────────────────▼───────────────────────────────┐
│  KREATE  (add_omega_graphics_module SHARED)        │
│    Kreate::App          — app lifecycle            │
│    Kreate::Window       — platform window shim     │
...
app/        — entry point template (main() → Kreate::App)
...
**Public interface** (single header, `include/kreate/Window.h`):
...
Wire `KreateWindow::nativeHandle()` into a `NativeRenderTargetDescriptor`:
...
kreate/
  ...
  src/
    App.cpp                     # Kreate::App implementation
    platform/
      Win32Window.cpp
      CocoaWindow.mm
      X11Window.cpp
  app/
    Main.cpp                    # Entry point template: main() → Kreate::App
  tests/
...
```cmake
omega_graphics_project(KREATE VERSION 0.1 LANGUAGES CXX OBJCXX)
...
4. **BasicGame.cpp** — test that uses Kreate::App to open a window and clear-color loop
