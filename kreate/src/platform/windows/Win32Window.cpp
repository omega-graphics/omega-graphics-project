...
static const wchar_t *KREATE_WINDOW_CLASS = L"KreateWindowClass";
static bool sClassRegistered = false;

static LRESULT CALLBACK KreateWndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    if (msg == WM_CLOSE) {
...
std::unique_ptr<Window> Window::create(const WindowDesc &desc) {
    if (!sClassRegistered) {
        ...
        wc.lpfnWndProc = KreateWndProc;
        wc.hInstance = GetModuleHandleW(nullptr);
        wc.hCursor = LoadCursor(nullptr, IDC_ARROW);
        wc.lpszClassName = KREATE_WINDOW_CLASS;
        RegisterClassExW(&wc);
    ...
    window->impl->hwnd = CreateWindowExW(
        0, KREATE_WINDOW_CLASS, wTitle.c_str(),
        WS_OVERLAPPEDWINDOW,
