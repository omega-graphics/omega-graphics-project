...
@interface KreateMetalView : NSView
@end
...
@interface KreateWindowDelegate : NSObject <NSWindowDelegate>
@property (nonatomic) BOOL shouldClose;
@end

@implementation KreateWindowDelegate
...
struct Window::Impl {
    NSWindow *nsWindow;
    KreateMetalView *metalView;
    KreateWindowDelegate *delegate;
    unsigned w;
    unsigned h;
};
...
std::unique_ptr<Window> Window::create(const WindowDesc &desc) {
    ...
    window->impl->delegate = [[KreateWindowDelegate alloc] init];
    [window->impl->nsWindow setDelegate:window->impl->delegate];

    window->impl->metalView = [[KreateMetalView alloc] initWithFrame:frame];
