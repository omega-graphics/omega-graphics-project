...
- If there is a bug:
    It could either be a bug in the code or an architectural design flaw. If the code logic looks correct but fails to fix the issue, consider either a design change to the system, or targeted research as the issue could be subtle that it requires a slight design change.