...
# Code Authoring
   ...
   - 5 Implement Incrementally.
    ...
    Plan location and lifecycle. All plan markdowns for the current module live under that module's `.plans/` subfolder (e.g. `gte/.plans/`, `aqua/.plans/`, `wtk/.plans/`). The folder has two lifecycle subdirectories:
    ...
    Active, in-progress plans stay at the top level of `.plans/`. Always look in `.plans/` first when checking for an existing plan before writing a new one; check `.plans/stale/` and `.plans/done/` only when you need historical context.
