...
# Code Authoring
   ...
   - 5 Implement Incrementally.
    ...
    Exception — small features. A phase under ~300 lines of code is a smaller feature, not a large-scoped feature/subsystem, and does NOT need its own multi-step phase breakdown (no 4c.6.1 / 4c.6.2 / 4c.6.3 sub-bullets). The full multi-phase plan is for large-scoped features and whole subsystems where the breakdown is what lets the change be reviewed in reviewable increments. For a small feature, a short one-paragraph note in the plan doc describing what the change does and why is enough — then implement directly. The 300-line ceiling is on the code change itself; the doc note + verification surface around it don't count. If you find yourself writing 4c.X.1 / 4c.X.2 / 4c.X.3 sub-bullets for a change that touches three files and ~70 lines, you're over-engineering — collapse the breakdown into a single note and move on.

    When a plan doc already exists. Many production features in this repo are tracked by a dedicated plan doc (e.g. `gte/.plans/Mesh-Shader-Implementation-Plan.md`, `gte/.plans/Raytracing-Full-Implementation-Plan.md`, `gte/.plans/Pipeline-Completion-Extension-Plan.md`). Do NOT write a parallel plan in chat — read the existing one first. If the sub-phase you are about to implement already has its own breakdown there, follow it. If it does NOT, ADD the implementation phasing to the existing plan doc (under the matching Phase section), then implement against your own addition. The plan doc is the source of truth; the chat is the working surface.

    Plan location and lifecycle. All plan markdowns for the current module live under that module's `.plans/` subfolder (e.g. `gte/.plans/`, `aqua/.plans/`, `wtk/.plans/`). The folder has three lifecycle subdirectories:
    - `.plans/stale/` — plans that have been superseded by a newer plan. Move the old doc here when its approach is replaced; do not delete it (history matters for understanding why the new plan exists).
    - `.plans/done/` — plans that have been implemented end-to-end. Move the doc here only when every phase is complete and shipped, not when the last phase merges but follow-ups remain open.
    - `.plans/future/` — forward-looking plans for work that is proposed but not yet scheduled to start. Distinct from an active plan at the top level (which is being implemented now): a future plan is parked until the work is greenlit — it may still be under discussion, gated on a feasibility spike, or waiting on a dependency. Move the doc up to the top level of `.plans/` when implementation actually begins.
    Active, in-progress plans stay at the top level of `.plans/`. Always look in `.plans/` first when checking for an existing plan before writing a new one; check `.plans/stale/`, `.plans/done/`, and `.plans/future/` only when you need historical or forward-looking context.
