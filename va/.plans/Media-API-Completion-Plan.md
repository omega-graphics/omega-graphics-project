...
## Phase 2: Linux (FFmpeg + Vulkan Video via OmegaGTE) — Full Implementation — ❌ NOT STARTED
...
>
> **Architectural cut (2026-05-17):** dropped VA-API as the HW accel path in favor of **Vulkan Video** (`AV_HWDEVICE_TYPE_VULKAN`), sharing the `VkInstance` / `VkPhysicalDevice` / `VkDevice` / video queue family from **OmegaGTE**'s Vulkan backend (`gte/src/vulkan/GEVulkan.{h,cpp}`). Rationale:
> - **Single GPU device across the process.** WTK's render path already runs on OmegaGTE's `VkDevice`. Running decode on a separate VA-API device forces a CPU bounce per frame (VA surface → `sws_scale` → upload to GTE texture). Sharing the Vulkan device means decoded frames land in a `VkImage` that GTE can sample directly — zero-copy from decoder to render target.
> - **Cross-vendor.** VA-API is Intel/AMD-first; NVIDIA support is fragmented. Vulkan Video (KHR extensions: `VK_KHR_video_queue`, `VK_KHR_video_decode_{h264,h265,av1}`, `VK_KHR_video_encode_{h264,h265,av1}`) is the cross-vendor path on Linux and aligns with the rest of the project's "Vulkan everywhere on Linux" stance.
> - **FFmpeg has first-class support.** Since FFmpeg 7.0+, `hwaccel=vulkan` covers H.264/H.265/AV1 decode; 7.1+ adds AV1 encode and H.264/H.265 encode is in-tree. `AVVulkanDeviceContext` lets us hand FFmpeg an externally-created device instead of having it open its own.
>
> The GTE-side enablement (video queue family selection, `VK_KHR_video_*` extension wiring, `VkImage` interop with `GETexture`) is tracked in a sibling plan: **[gte/docs/Vulkan-Video-Encode-Decode-Plan.md](../../gte/docs/Vulkan-Video-Encode-Decode-Plan.md)**. Phase 2 below depends on that plan landing at least the device-creation pieces.
...
```
                    ┌────────────────────────────────────────────┐
                    │ OmegaGTE (Vulkan backend)                  │
                    │   VkInstance / VkPhysicalDevice / VkDevice │
                    │   video decode + encode queue families     │
                    │   VK_KHR_video_{queue,decode_*,encode_*}   │
                    └──────────────────┬─────────────────────────┘
                                       │ shared via OmegaGTE C++ API
                                       ▼
FFmpegAudioVideoProcessor
  ├─ libavcodec     (encode/decode via AVCodecContext)
  ├─ libavformat    (demux/mux containers)
  ├─ libavutil      (frame allocation, AVVulkanDeviceContext)
  ├─ libswscale     (color space conversion fallback; preferred path stays on GPU)
  ├─ libswresample  (audio resampling)
  └─ Vulkan Video   (HW decode/encode when useHardwareAccel=true,
                     using OmegaGTE's VkDevice — zero-copy to GETexture)
...
class AudioVideoProcessor {
    ...
    AVBufferRef *hwDeviceCtx = nullptr;   // wraps OmegaGTE's VkDevice
    AVBufferRef *hwFramesCtx = nullptr;   // Vulkan frames pool (decoder output)
    SwsContext *swsCtx = nullptr;          // fallback only — preferred path is GPU
    SwrContext *swrCtx = nullptr;

    bool useHardwareAccel;
    OmegaGTE::SharedHandle<OmegaGTE::GEDevice> gteDevice;   // shared with renderer
    MediaCodecID currentEncodeCodec = MediaCodecID::H264;
    MediaCodecID currentDecodeCodec = MediaCodecID::H264;

public:
    explicit AudioVideoProcessor(bool useHardwareAccel,
                                 OmegaGTE::SharedHandle<OmegaGTE::GEDevice> gteDevice);
    ~AudioVideoProcessor();
...
2. If `useHardwareAccel`:
   - Allocate an `AVHWDeviceContext` of type `AV_HWDEVICE_TYPE_VULKAN` via `av_hwdevice_ctx_alloc(AV_HWDEVICE_TYPE_VULKAN)`. **Do not** call `av_hwdevice_ctx_create` — we want to hand FFmpeg our own device, not let it open one.
   - Cast `ctx->hwctx` to `AVVulkanDeviceContext *` and populate from the GTE Vulkan backend (see [Vulkan-Video-Encode-Decode-Plan §3](../../gte/docs/Vulkan-Video-Encode-Decode-Plan.md) for the accessor API):
     - `inst`, `phys_dev`, `act_dev`, `device_features`, `enabled_inst_extensions`, `enabled_dev_extensions`
     - `queue_family_index` / `nb_graphics_queues`, `queue_family_tx_index`, `queue_family_comp_index`
     - `queue_family_decode_index` / `nb_decode_queues`, `queue_family_encode_index` / `nb_encode_queues` (new fields the GTE plan must expose)
     - `get_proc_addr` = `vkGetInstanceProcAddr`
   - `av_hwdevice_ctx_init(hwDeviceCtx)` to finalize.
3. Retain the GTE device handle — it owns the `VkDevice` lifetime; the `AVBufferRef` is a non-owning view.
...
2. If hardware-accelerated:
   ...
   - Set the `get_format` callback to select `AV_PIX_FMT_VULKAN` from the offered list. Reject if the decoder doesn't offer it (means the codec/profile isn't supported by Vulkan Video on this device — fall back to software).
   - Allocate an `AVHWFramesContext` (`av_hwframe_ctx_alloc(hwDeviceCtx)`), set `format = AV_PIX_FMT_VULKAN`, `sw_format = AV_PIX_FMT_NV12` (or `P010` for 10-bit HDR), `width`/`height` from `VideoStreamDesc`. Init and attach via `decodeCtx->hw_frames_ctx`.
3. If software:
   - Standard `avcodec_find_decoder` + `avcodec_open2`. `SwsContext` for RGBA conversion.
...
4. If Vulkan-accelerated:
   - The frame's `data[0]` is an `AVVkFrame *` — it already holds the decoded `VkImage`(s). **Do not** call `av_hwframe_transfer_data`; that defeats the zero-copy point.
   - Wrap the `AVVkFrame` in a GTE-visible texture via the OmegaGTE interop API (`GEDevice::wrapExternalVkImage` — see GTE plan §4). The resulting `GETextureRef` is stored on `VideoFrame::videoFrame` (this requires `VideoFrame` to grow a `gteTexture` field; see Phase 5 API alignment).
   - Synchronization: FFmpeg signals a `VkSemaphore` on the `AVVkFrame` when decode completes; the GTE consumer must wait on it before sampling. The interop wrapper plumbs this through GTE's existing semaphore primitives.
5. If software: `sws_scale` to RGBA, copy to `VideoFrame::videoFrame.data`.
6. Populate header fields. Set `decodeFinishTime` to `std::chrono::high_resolution_clock::now()`.

**`encodeFrame`** follows the inverse:
- HW path: GTE-produced `VkImage` → wrap as `AVVkFrame` → `avcodec_send_frame` → `avcodec_receive_packet` → copy bitstream to `MediaBuffer`. Encoder signals semaphores when the input image can be released back to GTE.
- SW path: RGBA → `sws_scale` to encoder's pixel format → `avcodec_send_frame` → `avcodec_receive_packet`.
...
3. `start`: Register with the `PlaybackDispatchQueue`. The queue thread:
   ...
   - Decodes video frames via `AudioVideoProcessor` (with Vulkan Video via OmegaGTE's shared device if `useHardwareAccel`; software fallback otherwise — see Phase 2.6).
...
### 2.6 Vulkan Video Hardware Acceleration Path (via OmegaGTE)

When `useHardwareAccel = true`:

1. **Init**: Take the shared `OmegaGTE::GEDevice` (which owns the `VkInstance`, `VkPhysicalDevice`, `VkDevice`, and queue families). Populate `AVVulkanDeviceContext` from it as described in 2.2 and call `av_hwdevice_ctx_init`. No new DRM node opens — we reuse the device the renderer is already on.
2. **Decode**: `AVCodecContext::hw_device_ctx` points at the shared Vulkan device; `hw_frames_ctx` owns a `VkImage` pool sized for the stream. Decoded frames arrive as `AV_PIX_FMT_VULKAN` (`AVVkFrame` wrapping `VkImage` + `VkSemaphore`s).
3. **Zero-copy to renderer**: The decoded `VkImage` is wrapped as a `GETexture` via `GEDevice::wrapExternalVkImage` (defined by the GTE-side plan). The frame sink can sample it directly — no GPU→CPU copy, no `sws_scale` on the hot path.
4. **Synchronization**: `AVVkFrame` carries a `VkSemaphore` that FFmpeg signals when decode completes; the GTE consumer waits on it in its command buffer before sampling. Likewise for encode in the opposite direction.
5. **Encode**: GTE-produced render targets (already `VkImage`s) are wrapped as `AVVkFrame` and submitted to the encoder. Same semaphore dance.
6. **Software fallback**: If the codec/profile isn't supported by Vulkan Video on this device (e.g., AV1 encode on older hardware), the `get_format` callback declines `AV_PIX_FMT_VULKAN` and we fall through to software decode → `sws_scale` → CPU `VideoFrame`. This keeps behavior consistent across hardware tiers.
7. **GTE device prerequisite**: OmegaGTE must request the video queue families and `VK_KHR_video_*` extensions at device creation time. If the GTE device wasn't built with those (e.g., a headless test that doesn't need media), `useHardwareAccel = true` must degrade to software with a one-time warning rather than failing hard.
...
pkg_check_modules(FFMPEG REQUIRED
    libavcodec      # >= 7.0 for Vulkan Video decode; >= 7.1 for AV1 encode
    libavformat
    libavutil
    libswscale
    libswresample
...
# Vulkan loader/headers come transitively through OmegaGTE — no direct find_package(Vulkan)
# here. The FFmpeg backend uses Vulkan only via the AVVulkanDeviceContext handed in from
# GTE; it never opens its own VkInstance/VkDevice.
pkg_check_modules(PULSE libpulse-simple)  # optional, fallback to ALSA

# OmegaVA depends on OmegaGTE on Linux for the Vulkan Video HW accel path (and macOS/Windows
# for the GPU types — see Phase 6.7). Linked unconditionally.
target_include_directories(omegaWTK PRIVATE ${FFMPEG_INCLUDE_DIRS})
target_link_libraries(omegaWTK PRIVATE ${FFMPEG_LIBRARIES} OmegaGTE)
...
> The `omegaWTK` target above is the pre-Phase-6 placeholder. Once OmegaVA is extracted (Phase 6), these directives move to `va/CMakeLists.txt` and the target becomes `OmegaVA`.
...
| Platform | External Libraries | Internal |
|---|---|---|
| Linux | libavcodec ≥ 7.0, libavformat, libavutil, libswscale, libswresample, libavdevice, libpulse-simple (or ALSA). Vulkan loader/headers come transitively through OmegaGTE — no direct `find_package(Vulkan)`. | **OmegaGTE** (shared `VkDevice` + video queue families for Vulkan Video HW accel) |
| macOS | AVFoundation, AVFAudio, CoreVideo, VideoToolbox, CoreMedia (all system frameworks, already imported) | **OmegaGTE** (shared `MTLDevice` for `CVPixelBuffer`/`MTLTexture` interop on the processor) |
| Windows | Media Foundation, D3D11, D3D12 (already linked via pragma comments) | **OmegaGTE** (shared `ID3D12Device` for MFT ↔ D3D12 resource interop; see Phase 4 / 5 future work) |

The OmegaGTE dependency is uniform across platforms — see [gte/docs/Vulkan-Video-Encode-Decode-Plan.md](../../gte/docs/Vulkan-Video-Encode-Decode-Plan.md) for the GTE-side API (`VideoStreamDesc` / `PixelFormat` type sharing, `GEDevice::wrapExternal*Image`, video queue family exposure).
...
OmegaCommon ─┬─────────────────────────────────────┐
             ▼                                     ▼
         OmegaGTE ──────────────────────────→  OmegaVA  (new top-level module under va/)
             │  (shared VkDevice / MTLDevice /     │
             │   ID3D12Device; Vulkan Video        │
             │   queue families; VideoStreamDesc / │
             │   PixelFormat type sharing)         │
             ▼                                     ▼
OmegaCommon ─→ OmegaWTK_Core ─→ ... ─→ OmegaWTK_Composition ─→ OmegaWTK_UI ─→ OmegaWTK_Widgets
                                                                  │
                                                                  └─→ links OmegaVA (consumes MediaInputStream / VideoPlaybackSession)
```

Two new arrows compared to the original Phase 6 sketch:

- **OmegaGTE → OmegaVA** (new). OmegaVA's `AudioVideoProcessor` needs the same GPU device the renderer uses for zero-copy decode/encode. Specifically: on Linux, OmegaGTE's Vulkan backend supplies `VkInstance` / `VkPhysicalDevice` / `VkDevice` and the video queue families to FFmpeg via `AVVulkanDeviceContext` (see Phase 2.6). On macOS, the `MTLDevice` flows to `VTDecompressionSession` / `VTCompressionSession`. On Windows, the `ID3D12Device` will flow to MFTs once Phase 4/5 add the interop path. OmegaGTE also owns the cross-backend GPU pixel-format / texture descriptor types that OmegaVA's `VideoStreamDesc` will reference.
- **OmegaVA already depended on OmegaCommon** — unchanged.

The OmegaVA → OmegaGTE direction (not the reverse) is what we want: video is a consumer of the GPU, not the other way around. OmegaGTE remains buildable without OmegaVA.

**Companion plan in OmegaGTE:** [gte/docs/Vulkan-Video-Encode-Decode-Plan.md](../../gte/docs/Vulkan-Video-Encode-Decode-Plan.md) tracks the GTE-side work — video queue family selection, `VK_KHR_video_*` extension wiring, `wrapExternalVkImage` / `wrapExternalMTLTexture` / `wrapExternalD3D12Resource` APIs, and the shared type surface OmegaVA consumes.
...
7. **OmegaGTE dependency surface.** OmegaVA needs (a) the shared GPU device handle, (b) `wrapExternal*Image` interop APIs, and (c) shared pixel-format / texture descriptor types. Should OmegaVA's public headers expose `OmegaGTE::GEDevice` and `GETextureRef` directly (tightest coupling, zero-copy renderer integration is trivial), or should there be a thin `OmegaVA::GPUDeviceHandle` opaque wrapper (looser, but every WTK consumer has to unwrap twice)? The plan below assumes direct exposure — re-cut here changes the public API surface.

Phase 6.3 onwards assumes a baseline of: **namespace `OmegaVA`, include path `omegaVA/Media/...`, VideoFrameSink stays in OmegaVA, Composition::Rect leak removed in favor of `OmegaVA::FrameSize`, Android stays under WTK Mobile but links OmegaVA, CMake + AUTOM updated together, OmegaVA's public headers expose OmegaGTE types directly.** Any of those can be re-cut without changing the file moves.
...
1. **New file `va/CMakeLists.txt`** — modeled on the existing `wtk/CMakeLists.txt` Media section (lines ~186–296):
   ...
   - `target_include_directories(OmegaVA PUBLIC ${OMEGAVA_PUBLIC_INCLUDE_DIR} $<TARGET_PROPERTY:OmegaCommon,INCLUDE_DIRECTORIES> $<TARGET_PROPERTY:OmegaGTE,INCLUDE_DIRECTORIES>)`.
   - `target_link_libraries(OmegaVA PUBLIC OmegaCommon OmegaGTE)`. No link against OmegaWTK_* anything. OmegaGTE is `PUBLIC` because OmegaVA's public headers (e.g. `MediaPlaybackSession.h`'s `VideoFrame` carrying a `GETextureRef`, and the processor constructor taking a `SharedHandle<GEDevice>`) expose GTE types in their signatures.
...
2. **Top-level `CMakeLists.txt`** — ordering becomes `add_subdirectory(common)` → `add_subdirectory(gte)` → `add_subdirectory(va)` → `add_subdirectory(wtk)`. `va` must come after `gte` (it links it) and before `wtk` (which links `va`).
...
2. **OmegaGTE prerequisite landings** — at minimum the device-creation pieces from [gte/docs/Vulkan-Video-Encode-Decode-Plan.md](../../gte/docs/Vulkan-Video-Encode-Decode-Plan.md) (video queue family selection + `wrapExternalVkImage`) must exist on the OmegaGTE side before OmegaVA can link `PUBLIC OmegaGTE` without dead symbols. The full GTE plan can ship in parallel with Phase 2; only the device handle accessors are blocking.
3. **Open-decision confirmation** (6.2) — user signs off on namespace / include path / VideoFrameSink / mobile / single-vs-staged build-system / GTE coupling depth.
4. **CMake + source move** — single mechanical PR: move files, rewrite namespace, add `va/CMakeLists.txt`, edit top-level CMakeLists (insert `add_subdirectory(va)` between `gte` and `wtk`), drop OmegaWTK_Media. Verify all three platform builds (build verification per user preference, before merging).
5. **AUTOM mirror** — if staged, follow up here.
6. **Documentation move** — relocate this plan to `va/docs/`, leave a one-line forwarder in `wtk/docs/` for one release. The link to `gte/docs/Vulkan-Video-Encode-Decode-Plan.md` becomes a sibling-level `../../gte/docs/...` path (unchanged) once the plan moves.
7. **Test split** — opportunistic; can ship before or after the move.
