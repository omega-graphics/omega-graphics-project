...
@interface OmegaVAAVFAudioCaptureSampleBufferDelegate : NSObject <AVCaptureAudioDataOutputSampleBufferDelegate> {
    void *context;
    BOOL preview;
    BOOL videoDelegate;
}
...
@interface OmegaVAAVFVideoCaptureSampleBufferDelegate : NSObject <AVCaptureVideoDataOutputSampleBufferDelegate>  {
    void *context;
    BOOL preview;
}
...
@interface OmegaVAAVFCaptureFileRecordingDelegate : NSObject <AVCaptureFileOutputRecordingDelegate>  {
    void *context;
}
...
namespace OmegaVA {
    ...
    struct AVFAudioCaptureSession : public AudioCaptureSession {
        ...
        AVSampleBufferAudioRenderer *audioRenderer;
        OmegaVAAVFAudioCaptureSampleBufferDelegate *audioCaptureDelegate,*audioPreviewDelegate;
        ...
        explicit AVFAudioCaptureSession(AVFAudioCaptureDevice *device){
            ...
            audioPreviewDelegate = [[OmegaVAAVFAudioCaptureSampleBufferDelegate alloc]
                                    initWithCppContext:this videoCaptureDelegate:NO preview:YES];
            audioCaptureDelegate = [[OmegaVAAVFAudioCaptureSampleBufferDelegate alloc]
    ...
    struct AVFVideoCaptureSession : public VideoCaptureSession {
        ...
        OmegaVAAVFVideoCaptureSampleBufferDelegate *videoCaptureDelegate;
        OmegaVAAVFVideoCaptureSampleBufferDelegate *videoPreviewDelegate;
        OmegaVAAVFAudioCaptureSampleBufferDelegate *audioCaptureDelegate;
        ...
        explicit AVFVideoCaptureSession(AVFVideoDevice *videoDevice, AVFAudioCaptureDevice *audioDevice){
            ...
            videoPreviewDelegate = [[OmegaVAAVFVideoCaptureSampleBufferDelegate alloc]
                                    initWithCppContext:this preview:YES];
            videoCaptureDelegate = [[OmegaVAAVFVideoCaptureSampleBufferDelegate alloc]
...
@implementation OmegaVAAVFAudioCaptureSampleBufferDelegate
- (instancetype)initWithCppContext:(void *)context videoCaptureDelegate:(BOOL)videoDelegate preview:(BOOL)preview {
    if(self = [super init]){
...
- (void)captureOutput:(AVCaptureOutput *)output didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer fromConnection:(AVCaptureConnection *)connection {
    if(videoDelegate){
        ...
        auto session = (OmegaVA::AVFAudioCaptureSession *)context;
        if(preview){
...
@implementation OmegaVAAVFVideoCaptureSampleBufferDelegate
- (instancetype)initWithCppContext:(void *)context preview:(BOOL)preview {
    if(self = [super init]){
        self->context = context;
        self->preview = preview;
...
- (void)captureOutput:(AVCaptureOutput *)output didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer fromConnection:(AVCaptureConnection *)connection {
    auto session = (OmegaVA::AVFVideoCaptureSession *)context;
    if(preview){
        CVImageBufferRef imgBuffer = CMSampleBufferGetImageBuffer(sampleBuffer);
        auto frame = std::make_shared<OmegaVA::VideoFrame>();
        OmegaVA::TimePoint timePoint;
        CMTime t = CMSampleBufferGetOutputDecodeTimeStamp(sampleBuffer);
        OmegaVA::CMTimeToTimePoint(t,timePoint);
...
@implementation OmegaVAAVFCaptureFileRecordingDelegate
- (instancetype)initWithCppContext:(void *)context {
    if(self = [super init]){
        self->context = context;
    }
