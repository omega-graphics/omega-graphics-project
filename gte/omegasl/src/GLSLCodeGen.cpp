...
namespace omegasl {
    ...
    class GLSLCodeGen : public CodeGen {
        ...
        OmegaCommon::Map<OmegaCommon::String, ast::StructDecl *> structDeclMap;
    ...
    public:
                ...
                case SHADER_DECL : {
                    ...
                    for(auto & resource : _decl->resourceMap){
                        ...
                        if(type_ == ast::builtins::buffer_type){
                            ...
                            shaderOut << "buffer "; writeGLSLIdent(res_decl->name, shaderOut); shaderOut << "_Layout" << std::endl;
                            shaderOut << "{" << std::endl;
                            writeTypeExpr(res_decl->typeExpr->args[0],shaderOut);
                            shaderOut << " "; writeGLSLIdent(res_decl->name, shaderOut); shaderOut << "[];" << std::endl;
                            shaderOut << "};" << std::endl;
                        }
                            ...
                            shaderOut << "layout(set = "<< set <<",binding =" << binding << ") uniform sampler "; writeGLSLIdent(res_decl->name, shaderOut); shaderOut << ";" << std::endl;
                        }
                    ...
                    for(auto stmt_it = _decl->block->body.begin();stmt_it != _decl->block->body.end();stmt_it++){
                        ...
                        if(stmt->type == RETURN_DECL){
                            ...
                            if(_return_stmt->expr && _decl->shaderType == ast::ShaderDecl::Fragment) {
                                ...
                                /// Hull/Domain: main() is void in GLSL — assign Position field to gl_Position output.
                                ...
                                if(structIt != structDeclMap.end()){
                                    for(auto & f : structIt->second->fields){
                                        if(f.attributeName.has_value() && f.attributeName.value() == ATTRIBUTE_POSITION){
                                            posField = f.name;
                                            break;
                                        }
                                    }
                                ...
                                if(_decl->shaderType == ast::ShaderDecl::Hull){
                                    shaderOut << "gl_out[gl_InvocationID].gl_Position = ";
                                } else {
        ...
        void generateExpr(ast::Expr *expr) override{
            switch (expr->type) {
                ...
                case ID_EXPR : {
                    ...
                    writeGLSLIdent(_expr->id, shaderOut);
                    break;
                }
                case UNARY_EXPR : {
        ...
        bool compileShader(ast::ShaderDecl::Type type, const OmegaCommon::StrRef &name, const OmegaCommon::FS::Path &path, const OmegaCommon::FS::Path &outputPath) override {
            ...
            auto spvPath = OmegaCommon::FS::Path(outputPath).append(name).concat(".spv").absPath();
            auto srcPath = OmegaCommon::FS::Path(path).append(name).concat(file_ext).absPath();
