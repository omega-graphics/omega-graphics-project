...
> Updated 2026-07-13. Tracks against **Phasing** below.

### Phase 5 — Amplification / task stage: **code-complete and end-to-end verified on all three backends (Vulkan headless + windowed; D3D12 + Metal windowed via `MeshAndRaytracingTest` — racket renders identically)**

The mesh pipeline's upstream half is in. `amplification(x,y,z)` is a real OmegaSL stage, `in payload` / `out payload` cross the stage boundary, and `dispatchMesh(x, y, z, payload)` launches a *computed* number of mesh threadgroups from the GPU. All three backends build the two-stage PSO; all three grew a `bindResourceAtAmplificationShader` family.

Per project convention ([[feedback_mark_unverified_backends_in_plan]]): **Vulkan is verified end-to-end on the Linux host (NVIDIA RTX 5080), including the validation layers, which are silent. D3D12 and Metal are now also verified end-to-end** — the user ran `MeshAndRaytracingTest` on both backends (2026-07-13) and the racket renders identically to the Vulkan build, so DXC's `as_6_5` amplification profile and Metal's `[[object]]` object function both compiled and each driver accepted the two-stage PSO. "The source looks right" is now backed by "the driver accepted it and the frame is correct."

Verified in-session 2026-07-12:

- **`gte/tests/mesh_shader_test.cpp` Phase C** (new, headless, registered as `omegagte_mesh_shader`): one amplification threadgroup writes a blue tint into the payload and dispatches TWO mesh threadgroups; each child paints its own half of an 8×8 target, keyed by its child index. Readback: left `(0, 0, 229, 255)`, right `(255, 0, 229, 255)`. The blue 229 (= 0.9 × 255) on **both** halves is the payload check; the differing red channel is the child-index check; both halves being painted at all is the dispatch-count check. The test samples two pixels precisely so those three failure modes produce *different* wrong outputs instead of all collapsing to "black" — a payload-only test would pass against a dead child index, and vice versa.
- **`gte/tests/MeshAndRaytracingTest`** (windowed, shared `GTETestWindow` body, registered on all three backends) upgraded from one-mesh-threadgroup-per-triangle to a real two-stage dispatch. On the 389,392-triangle FBX the host now issues **12,169 amplification threadgroups instead of 389,392 mesh dispatches** — a 32× reduction in host-issued work, with the GPU deciding how many children each batch is worth. `makeMeshPipelineState (amplification + mesh) -> live PSO`; zero Vulkan validation errors. **Visually confirmed by the user on all three backends** (Vulkan on the Linux/RTX 5080 host; D3D12 and Metal on 2026-07-13 via the same windowed test): the racket renders identically to the pre-amplification build on each, which is the actual pass criterion — the per-triangle color ramp is keyed on the *payload-derived global* triangle index, so a payload that failed to cross the stage boundary would have rendered the model in visible 32-triangle stripes (every batch tinting 0..31 identically) rather than degrading to something obviously broken. "Looks the same" is the strong result here, not the weak one.
- Full `ctest`: `gte/tests` 15/15, `gte/omegasl/tests` 150/150. The OmegaSL count jumped from 99 because **the mesh fixtures were never registered in CTest at all** — Phases 2a/2b/2c verified `mesh_basic.omegasl` and the `invalid_set_mesh_outputs_*` / `invalid_topology_point` negatives by hand in-session and never wired them up, so the entire mesh codegen path had no standing regression coverage. Registered here alongside the §5 fixtures.
- GLSL amplification source compiles via `glslc -fshader-stage=task --target-env=vulkan1.2` and validates clean under `spirv-val`.

Files touched (~1400 lines across the front-end, three codegens, three runtimes, and the tests):

| Area | File | Change |
|---|---|---|
| Lexer / tokens | `Toks.def`, `Lexer.cpp` | `amplification` is a hard keyword; `payload` stays contextual (consumed only after an explicit `in`/`out`). |
| AST | `AST.h`, `AST.def`, `AST.cpp` | `ShaderDecl::Amplification` (tail-appended); `AttributedFieldDecl::PayloadKind`; `ampHasDispatchMeshCall`; `BUILTIN_DISPATCH_MESH`; new `ast::payloadStructSize()`. |
| Parser | `Parser.cpp` | `amplification(x=..,y=..,z=..)` descriptor (reuses the mesh key=value loop); `in payload` / `out payload` param qualifier. Note `sawAccessQualifier` — `In` is the DEFAULT access, so without it a bare `payload T p` would have been silently accepted as an input payload. |
| Sema | `Sema.h`, `Sema.cpp` | `AmplificationShaderArgument` attribute context; `dispatchMesh` stage/arity/type rules incl. "the 4th argument must BE the `out payload` param" (on GLSL/MSL the payload is not passed at the call site, so any other expression there would be silently dropped); payload structural gate shared by both stages; **`dispatchMesh` must be the final statement, exactly once** — the intersection of the three backends' rules, not any one of them (GLSL's `EmitMeshTasksEXT` terminates the shader; HLSL/Metal would accept a trailing statement). |
| Serialization | `omegasl.h`, `ShaderArchive.cpp` | `OMEGASL_SHADER_AMPLIFICATION`; `omegasl_payload_desc`; writer/reader arms. **`OMEGASLLIB_FORMAT_VERSION` 1 → 2** — the mesh record gained a trailing payload size, so a v1 archive with a mesh entry would have had 4 bytes of the *next* record misparsed as its payload size. The bump turns that silent desync into a clean rejection. |
| GLSL codegen | `GLSLTarget.cpp` | `.task` ext → `-fshader-stage=task`; `shaderc_glsl_task_shader`; file-scope `taskPayloadSharedEXT <T> <name>;` on BOTH stages (which is why the mesh body needs no member rerouting — `p.field` already resolves against the global); `dispatchMesh` → `EmitMeshTasksEXT(x,y,z)` with the payload arg dropped. |
| HLSL codegen | `HLSLTarget.cpp` | `as_6_5` profile; `[numthreads]`; `groupshared <T> <name>;` on the AS + a real `in payload <T> <name>` param on the MS; `dispatchMesh` → `DispatchMesh` (a plain rename — HLSL is the one backend that takes all four args). **Register space: amplification → `space2`.** Load-bearing, not cosmetic: an amp and a mesh that both declare `[in pc]` would otherwise emit two `register(b0, space0)` and D3D12 root-signature creation fails on the duplicate. |
| MSL codegen | `MSLTarget.cpp` | `[[object]]`; `object_data <T>& p [[payload]]` + a synthesized `mesh_grid_properties` handle on the object fn; `const object_data <T>& p [[payload]]` on the mesh fn; `dispatchMesh` → `<grid>.set_threadgroups_per_grid(uint3(...))` with the payload arg dropped (it is already bound by reference). |
| Public API | `GERenderTarget.h` | New `bindResourceAtAmplificationShader(GEBuffer\|GETexture\|GESamplerState)` pure virtuals. |
| Vulkan | `GEVulkan.{h,cpp}`, `GEVulkanPipeline.h`, `GEVulkanCommandQueue.{h,cpp}` | `taskShader` feature **queried, not assumed** (the extension mandates `meshShader` but leaves `taskShader` optional — hard-coding `VK_TRUE` would fail `vkCreateDevice` outright on a mesh-only device, taking the whole engine down); `VK_SHADER_STAGE_TASK_BIT_EXT` in the layout's descriptor + push-constant stage flags; TASK stage in the PSO; amp bind → descriptor **set 2**. |
| D3D12 | `GED3D12.cpp`, `GED3D12Pipeline.h`, `GED3D12CommandQueue.{h,cpp}` | `stream.AS`; root signature over `{mesh, fragment, amplification}` with `registerSpace = 2` for the amp; `setRenderConstants` + the three binds fan out to the amp's root params. |
| Metal | `GEMetal.mm`, `GEMetalPipeline.h`, `GEMetalCommandQueue.{h,mm}` | `objectFunction` + **`payloadMemoryLength`** (required whenever an object function is set — Metal does not derive it from the signature, and a 0 leaves the payload nowhere to live); `threadsPerObjectThreadgroup:` now reads the amp's real `threadgroupDesc` (Phase 4c pinned it at `(1,1,1)`, which would now silently run every amp group single-threaded); `setObjectBytes:` + the `setObject*` bind family with `MTLRenderStageObject` fences — 4c.5.1–4c.5.4 landed verbatim one stage over, exactly as that phase's trade-off note predicted. |
| Tests | `mesh_shader_test.cpp`, `MeshAndRaytracingTest/`, `amplification_basic.omegasl`, 4 × `invalid_*.omegasl`, both test `CMakeLists.txt` | Phase C; the windowed amp→mesh upgrade; the OmegaSL fixture + negatives; **CTest registration for the previously-unregistered mesh fixtures**. |

Bugs found and fixed while landing this (both latent, both pre-existing):

1. **Vulkan `setRenderConstants` pushed with the wrong stage bit on a mesh pipeline.** It derived the stage from the *slot* (`vertexShader` → `VK_SHADER_STAGE_VERTEX_BIT`), but on a mesh PSO that slot holds the MESH shader (the Phase-4a slot-doubling). So it pushed with `VERTEX` while the pipeline layout's range was built with `MESH` — a per-byte/per-stage mismatch (VUID-vkCmdPushConstants-offset-01795). It survived only because most drivers write the push block regardless of the stage mask; it was one strict driver away from a mesh shader silently reading a stale MVP. Fixed by deriving the bit from `omegasl_shader::type` (the ground truth) rather than from which slot the shader occupies. **Principle, and this is now the third instance** (4c.5's MSL `paramIndex` reset, 4a's texture-layout shadow): *slot-doubling saves plumbing but leaves the slot's NAME lying about its contents — every read that keys off the slot name rather than the value's own type is a latent bug waiting for a new stage to expose it.*
2. **The mesh-shader OmegaSL fixtures were never in CTest.** Not a code bug, but the same class of problem: three phases of mesh codegen shipped with zero standing regression coverage, verified only by hand in-session. Registered now.

Trade-offs / deferred under Phase 5:

- **D3D12 and Metal verified 2026-07-13.** Both toolchains and runtimes were exercised end-to-end via `MeshAndRaytracingTest` (racket renders identically to Vulkan). The two spots flagged as highest-risk both held: (a) D3D12 — DXC accepted the `groupshared` payload + `DispatchMesh` shape and the `space2` root signature built; (b) Metal — `payloadMemoryLength` matched what the compiled `[[object]]` function wanted, so the `payloadStructSize` over-approximation is safe in practice.
- **No threadgroup-shared compaction in the amplification stage.** A "cull and compact surviving meshlets" amp shader wants `threadgroup` scratch + a barrier + an atomic counter, but Sema currently restricts `threadgroup` VarDecls to compute shaders. Relaxing that to amplification (and mesh) is a small, separable front-end change; until it lands, amplification shaders dispatch a computed count without compacting a per-thread survivor list. Neither test needs it.
- **Payload size is over-approximated** (vec3 → 16B, struct → 16B multiple). Safe for its only consumer (Metal's `payloadMemoryLength`, an allocation size) and for the amp↔mesh agreement check (which only needs *a* number both sides compute identically). A backend that ever needs the exact logical size should get its own field rather than tightening this one.
- **The amp↔mesh payload check compares SIZE, not the struct.** Two different payload structs that happen to be the same size pass it. Cheap and catches the realistic error (mismatched types); a name/hash comparison would catch the pathological one, at the cost of interning a string through the archive.
...
- `mesh(max_vertices=N, max_primitives=M, topology=T)` — Phase 1/2
- `amplification(x, y, z)` — Phase 5

Status update: the `mesh` stage **front-end is implemented** (parse + Sema + serialization model; see Implementation Status). The **`amplification` stage is implemented on all three backends** (Phase 5, 2026-07-12) — front-end, codegen, PSO, dispatch, and resource binding — **verified end-to-end on Vulkan; unverified off-platform on D3D12 / Metal**. Mesh **codegen** is **live on GLSL, HLSL, and MSL** (Phase 2a + 2b + 2c) and now exercised end-to-end on **all three backends**: HLSL through D3D12 since Phase 4b, MSL through Metal since Phase 4c, GLSL through Vulkan since Phase 4a. **D3D12 backend is live** — Phase 4b shipped 2026-05-30; mesh PSO build via `CD3DX12_PIPELINE_MESH_STATE_STREAM` and `commandList->DispatchMesh` dispatch both pass an end-to-end visual test on an RTX 2080 Ti. **Metal backend is live** — Phase 4c shipped 2026-05-30; mesh PSO build via `MTLMeshRenderPipelineDescriptor` and `[rp drawMeshThreadgroups:...]` dispatch both pass a headless integration test on Apple M4 Max, closing the Phase 2c off-platform-unverified callout in the process. **Vulkan backend is live** — Phase 4a debugged + landed 2026-06-02 (commit `0c76e07`); mesh PSO build via `VkGraphicsPipeline` with `VK_SHADER_STAGE_MESH_BIT_EXT` and `vkCmdDrawMeshTasksEXT` dispatch both pass the shared `omegagte_mesh_shader` headless test (Phase A + Phase B + `#include` negative) on the Linux Vulkan host. Sema rejects `topology=point` as a portability gate (HLSL SM 6.5 has no `point` output topology) — see the Phase 2b status block + Cross-Backend Differences.
...
- **4a — Vulkan** *(verifiable on the project's native Linux Vulkan host per AGENTS.md "Building"; the agent itself is on macOS for this landing)* — **DONE 2026-06-02, debugged + passing on the Linux Vulkan host** (commit `0c76e07` "GTE: Vulkan Mesh Shader SUCCESS"): `VkGraphicsPipeline` with `VK_SHADER_STAGE_MESH_BIT_EXT` + fragment stage, empty `VkPipelineVertexInputStateCreateInfo` / `VkPipelineInputAssemblyStateCreateInfo` (spec-ignored when a mesh stage is present, but passed well-formed to keep validation layers quiet); `vkCmdDrawMeshTasksEXT` loaded from `VK_EXT_mesh_shader` at device init; `VkPhysicalDeviceMeshShaderFeaturesEXT { meshShader = VK_TRUE, taskShader = VK_FALSE }` chained into `vkCreateDevice`'s `pNext` when the extension is available; `createPipelineLayoutFromShaderDescs` now maps `OMEGASL_SHADER_MESH` → `VK_SHADER_STAGE_MESH_BIT_EXT` so descriptor-set bindings + push-constant ranges get the right stage flags (mesh-stage resource binding then "comes for free" through the existing `bindResourceAtVertexShader` path, just like D3D12). `GEVulkanRenderPipelineState` carries the same `isMesh` flag pattern Phase 4b/4c established so the command-buffer side asserts on dispatch kind. Amplification stage hard-stopped at `makeMeshPipelineState` with a precise Phase-5 diagnostic, matching the D3D12/Metal precedent.

  Two fixes the read-through landing missed, both caught by the Linux Vulkan validator on first run (see commit `0c76e07`):

  1. **`drawMeshTasks` was missing the pre-draw "pending state" flushes.** Every Vulkan draw entry point calls `beginRenderPassIfDeferred()` (`startRenderPass` defers `vkCmdBeginRenderPass` so bind-time image-layout barriers can land *outside* the render pass — they're illegal inside) and `bindDescriptorSetsIfPending()` (descriptor writes are batched per-draw). My initial `drawMeshTasks` body went straight from the asserts to `vkCmdDrawMeshTasksExt(...)` and the validator fired `VUID-vkCmdDrawMeshTasksEXT-renderpass`. Fix: the two pre-flush calls before the dispatch. **Principle for the next maintainer: pre-draw flushes are part of every Vulkan draw entry point's contract, not just `drawPolygons` — audit the surrounding draw functions when adding a new one.**
  2. **Pre-existing latent texture-layout-tracker fragmentation surfaced.** `GEVulkanTexture` carried two layout trackers: the canonical `tex.layout` (updated by every encoder path) and a `stagingCurrentLayout` shadow (only the immediate-upload / readback paths touched). For a `FromGPU` texture filled via `copyTextureToTexture(rt, readback)` and then read back via `getBytes`, the shadow stayed at `UNDEFINED` while the real layout had moved through `TRANSFER_DST_OPTIMAL` — the validator caught the disagreement on the staging-readback barrier (`UNASSIGNED-CoreValidation-DrawState-InvalidImageLayout`). Fix: drop `stagingCurrentLayout` entirely, route both staging paths through `tex.layout`, and broaden the source-side access masks to `MEMORY_READ_BIT | MEMORY_WRITE_BIT` (the prior op can be shader-sample, render-target write, or a prior transfer — narrow access masks fight that broadness). **Principle (this is the second time it has shown up — Phase 4c.5's MSL `paramIndex` reset was the first): latent state-tracker fragmentation surfaces under new workloads.** Phase B's mesh-shader → copyTextureToTexture → getBytes pipeline was the first to combine those three steps; compute/graphics tests had never tripped the shadow drift.

  Verified in commit `0c76e07` on the Linux Vulkan host. `mesh_shader_test.cpp` Phase A + Phase B + Negative (`#include` rejection) all pass; `gte/tests/vulkan/MeshShaderTest` registered as `omegagte_mesh_shader` in CTest.
...
### Phase 5 — Amplification / task stage (additive) — **DONE 2026-07-12; verified on all three backends 2026-07-13 (Vulkan; D3D12 + Metal via `MeshAndRaytracingTest`)**

See **Implementation Status → Phase 5** for the landing report, the two latent bugs it surfaced, and what still needs verifying on the other two backends.

The deferred payload machinery: `amplification(x,y,z)` descriptor, `in payload` / `out payload` params, payload-type matching between amp & mesh, dispatch-children (`EmitMeshTasksEXT` GLSL / `DispatchMesh` HLSL / object stage MSL), and the pipeline's optional amplification slot. A new `OMEGASL_SHADER_AMPLIFICATION` enum appends at the tail (the enum/serialization were laid out to allow this). Touches every layer again but is purely additive on top of Phases 1–4.

#### Surface (locked)

```omegasl
struct MeshletPayload {          // plain struct — no semantics, so not `internal`
    uint baseTriangle;
};
...
void ampFunc(uint3 gid : ThreadGroupID,
             out payload MeshletPayload p) {
    p.baseTriangle = gid.x * 32u;
    uint remaining = pc.triCount - p.baseTriangle;
    dispatchMesh(remaining < 32u ? remaining : 32u, 1u, 1u, p);
}

[in pc, in vertBuf]
mesh(max_vertices=3, max_primitives=1, topology=triangle)
void meshFunc(uint3 gid : ThreadGroupID,          // child index within the amp's dispatch
              in payload MeshletPayload p,
              out vertices MeshletVertex verts[3],
              out indices  uint3         tris[1]) { … }
```

`dispatchMesh(x, y, z, payload)` is the new mesh-stage-child dispatch builtin. It does NOT lower 1:1 — the payload argument is real on HLSL and dropped on the other two, because they address the payload through a shared declaration rather than a call argument:

| Backend | Native form | Payload lives in |
|---|---|---|
| GLSL (`GL_EXT_mesh_shader`) | `EmitMeshTasksEXT(x, y, z)` — **payload arg dropped** | file-scope `taskPayloadSharedEXT <Struct> <name>;` (declared identically on the task AND the mesh side) |
| HLSL (SM 6.5) | `DispatchMesh(x, y, z, <name>)` — **all four args** | file-scope `groupshared <Struct> <name>;` in the AS; `in payload <Struct> <name>` param on the MS |
| MSL (Metal 3) | `<mesh_grid_props>.set_threadgroups_per_grid(uint3(x, y, z))` — **payload arg dropped** | `object_data <Struct>& <name> [[payload]]` param on the object fn; `const object_data <Struct>& <name> [[payload]]` on the mesh fn |

Because `EmitMeshTasksEXT` / `DispatchMesh` are *terminating* / group-uniform calls on GLSL and HLSL respectively, Sema requires `dispatchMesh` to be the **final statement of the amplification body**, exactly once. That is a stricter rule than any single backend needs, and it is the rule that makes the same source portable to all three.

#### Implementation phasing

- **5.1 — OmegaSL front-end.** `amplification` becomes a hard keyword (`Toks.def` + `Lexer.cpp::isKeyword`), `ast::ShaderDecl::Amplification` appends at the tail of the stage enum, and `ast::AttributedFieldDecl::PayloadKind {NotPayload, InPayload, OutPayload}` joins `MeshOutputKind` (the `payload` word stays contextual — consumed only when it directly follows `in`/`out`, same trick `vertices`/`indices` use). The parser reuses the `mesh(...)` key=value descriptor loop for `amplification(x=..,y=..,z=..)` and stores the dims in `threadgroupDesc`, exactly as compute/mesh do. Sema adds `AttributeContext::AmplificationShaderArgument` (thread-IDs, like compute) and the structural gate: an amplification shader returns `void`, declares exactly one `out payload` param (a non-array struct), and calls `dispatchMesh` exactly once as its last statement; a mesh shader declares at most one `in payload` param. `dispatchMesh(uint,uint,uint,payload)` is a new reserved builtin (`AST.def`) validated for stage, arity, and arg types.
- **5.2 — Serialization + payload sizing.** `OMEGASL_SHADER_AMPLIFICATION` appends after `OMEGASL_SHADER_MESH` in `omegasl.h` (tail-append keeps every existing archive's numeric encoding intact). A new `omegasl_payload_desc { unsigned size; }` rides on `omegasl_shader` and is written for BOTH amplification and mesh entries (`size == 0` ⇒ "this stage has no payload"). `ShaderArchive.cpp` gets the matching writer/reader arm. The byte size is computed in the compiler by a new shared `ast::payloadStructSize(StructDecl*)` using conservative MSL/HLSL rules (natural alignment, `float3` = 16B, struct rounded to 16) — the only consumer is Metal's `payloadMemoryLength`, which is a threadgroup-memory *allocation* size, so over-approximating is safe on every backend. It doubles as the runtime amp↔mesh payload cross-check (see 5.6–5.8).
- **5.3 — GLSL codegen (task stage).** `shaderFileExt(Amplification) → ".task"` (so the shared derivation hands glslc `-fshader-stage=task`), `shaderc_glsl_task_shader` + the same Vulkan-1.2 / SPIR-V-1.4 pin the mesh stage already takes. Header emits `#extension GL_EXT_mesh_shader : require` + `layout(local_size_*) in;`; the payload param is suppressed from `void main()` and re-emitted at file scope as `taskPayloadSharedEXT <Struct> <name>;` — on the mesh side too, which is what makes `p.field` reads Just Work in the mesh body with no member rerouting. `renameBuiltin` maps `dispatchMesh` → `EmitMeshTasksEXT`, and a target override drops the 4th (payload) argument.
- **5.4 — HLSL codegen (amplification stage).** `[numthreads(x,y,z)]`, DXC profile `as_6_5`, payload param suppressed from the signature and re-emitted as file-scope `groupshared <Struct> <name>;`. The mesh side takes `in payload <Struct> <name>` as a real parameter. `renameBuiltin` maps `dispatchMesh` → `DispatchMesh` and the shared `(args)` print carries all four arguments unchanged. **Register space**: the amplification stage gets `space2`, mirroring the existing fragment=`space1` precedent. This is load-bearing, not cosmetic — without it an amp and a mesh that both declare `[in pc]` emit two `register(b0)` in `space0` and D3D12 root-signature creation fails on the duplicate register.
- **5.5 — MSL codegen (object stage).** `[[object]]` function decorator + `[[max_total_threads_per_threadgroup(N)]]`. The signature gains `object_data <Struct>& <name> [[payload]]` (from the suppressed `out payload` param) and a `mesh_grid_properties __omegasl_mesh_grid` handle; `dispatchMesh` lowers to `__omegasl_mesh_grid.set_threadgroups_per_grid(uint3(x,y,z))` with the payload arg dropped. The mesh entry gains `const object_data <Struct>& <name> [[payload]]` ahead of the existing `__omegasl_mesh_output_handle`.
- **5.6 — Vulkan runtime.** `VkPhysicalDeviceMeshShaderFeaturesEXT::taskShader` flips from the hard `VK_FALSE` Phase 4a pinned it at to the device's advertised value; `createPipelineLayoutFromShaderDescs` maps `OMEGASL_SHADER_AMPLIFICATION → VK_SHADER_STAGE_TASK_BIT_EXT` in BOTH the push-constant stage-flag union and the per-shader descriptor-set stage flags. `makeMeshPipelineState` adds a `VK_SHADER_STAGE_TASK_BIT_EXT` stage to the PSO when `amplificationFunc` is set. **Shader-array order is load-bearing**: the layout builder creates one descriptor set per shader *in array order*, and `bindResourceAtVertexShader` hardcodes set 0, so the array must stay `{mesh, fragment, amplification}` — appending the amp at the tail keeps set 0 = mesh and set 1 = fragment exactly where every existing bind expects them.
- **5.7 — D3D12 runtime.** `stream.AS = ampShader->shaderBytecode` on the existing `CD3DX12_PIPELINE_MESH_STATE_STREAM`; root signature built over `{mesh, fragment, amplification}` with `registerSpace = 2` for the amp stage (matching 5.4). `GED3D12RenderPipelineState` gains an `amplificationShader` slot so `setRenderConstants` can push root constants to the AS's root parameter alongside the MS's and PS's — the same fan-out it already does for vertex + fragment. `DispatchMesh(x,y,z)` is unchanged: its group counts already mean "amplification threadgroups" when an AS is bound.
- **5.8 — Metal runtime.** `pipelineDesc.objectFunction` + `pipelineDesc.payloadMemoryLength` (from 5.2's serialized size) on `MTLMeshRenderPipelineDescriptor`. `GEMetalRenderPipelineState` gains an `amplificationShader` slot; `drawMeshTasks` reads its `threadgroupDesc` for the `threadsPerObjectThreadgroup:` argument that Phase 4c had pinned at `(1,1,1)` precisely because no object stage could be bound. `setRenderConstants` gains a `setObjectBytes:` arm, the object-stage twin of the `setMeshBytes:` routing 4c.5.4 landed.
- **5.9 — Verification.** Headless: `gte/tests/mesh_shader_test.cpp` gains a **Phase C** — an amplification shader writes a color index into the payload and dispatches one mesh group; the mesh shader colors its triangle *from the payload*, so the readback center pixel is the payload-derived color and a payload that never arrived reads back as a visibly different pixel (not merely a crash). Windowed: `MeshAndRaytracingTest` (already a shared `GTETestWindow` body registered on all three backends) is upgraded from "one mesh threadgroup per triangle" to a real two-stage amp→mesh dispatch — one amplification threadgroup per 32-triangle batch, payload carrying the batch's base triangle, and a *variable* child count on the tail batch (`triCount % 32`). That variable count is the part that proves the payload plumbing: with a dead payload every batch would render triangles 0–31 and the racket's silhouette would collapse visibly rather than fail silently.

- **5.10 — Amplification-stage resource binding (`bindResourceAt*AmplificationShader`).** A NEW public bind family on `GECommandBuffer`, alongside the existing vertex/fragment ones: `bindResourceAtAmplificationShader(SharedHandle<GEBuffer>&, unsigned)` + the `GETexture` (with swizzle) and `GESamplerState` overloads. `setRenderConstants` additionally fans out to the amplification stage, the same way it already fans out to vertex + fragment.

  **Why a new method here, when Phase 4c explicitly refused to add `bindResourceAtMeshShader`.** Those two cases are not the same shape, and the earlier refusal does not generalize. Mesh *replaces* the vertex stage — it lands in the `vertexShader` base slot (the 4b.1/4c.1/4a slot-doubling), so `bindResourceAtVertexShader` is already addressing the right shader and only needed to re-route the backend call underneath; adding a second public method would have given two spellings for one slot and diverged the API on Metal alone. Amplification *coexists* with mesh: it is a genuinely additional stage, with its own resource table, its own descriptor set, and its own register space. There is no slot to double onto, so there is nothing for an existing method to route — the resource simply has no way to name the amplification stage without a method that says so. The precedent that actually governs is `bindResourceAtFragmentShader`, which exists for exactly this reason.

  Per backend: **Vulkan** — writes descriptor set 2 (the shader array is `{mesh, fragment, amplification}`, and the layout builder makes one set per shader in array order, so the amp's set index is 2 by construction; `bindResourceAtVertexShader` keeps set 0 and `bindResourceAtFragmentShader` keeps set 1, untouched). **D3D12** — `getRootParameterIndexOfResource` against the amp's `omegasl_shader` at `space2` (5.7), under the root signature's existing `D3D12_SHADER_VISIBILITY_ALL`. **Metal** — `setObjectBuffer:` / `setObjectTexture:` / `setObjectSamplerState:` / `setObjectBytes:`, with `MTLRenderStageObject` fences on the buffer + texture paths; this is 4c.5.1–4c.5.4 landed verbatim with the Object selectors, which is precisely what the 4c.5 trade-off note said would happen "when Phase 5 wires amplification."

Trade-offs / deferred under Phase 5:

- **No threadgroup-shared compaction in the amplification stage.** A "cull and compact surviving meshlets" amp shader wants `threadgroup` scratch + a barrier + an atomic counter, but Sema currently restricts `threadgroup` VarDecls to compute shaders. Relaxing that to amplification (and mesh) is a small, separable front-end change; until it lands, amplification shaders dispatch a computed count without compacting a per-thread survivor list. The 5.9 test is written to need neither.
- **Payload size is over-approximated.** `payloadStructSize` rounds `float3` to 16B and the struct to a 16B multiple. Metal's `payloadMemoryLength` is an allocation, so a larger-than-needed value is safe; it is never used to index into the payload on any backend. If a future backend needs the *exact* logical size, it should get its own field rather than tightening this one.
