...
> Updated 2026-07-12. Tracks against **Phasing** below.

### Phase 5 — Amplification / task stage: **code-complete on all three backends; end-to-end verified on Vulkan (headless + windowed); D3D12 / Metal compile-and-run unverified off-platform**
...
Per project convention ([[feedback_mark_unverified_backends_in_plan]]): **Vulkan is verified end-to-end on the Linux host (NVIDIA RTX 5080), including the validation layers, which are silent. D3D12 and Metal are code-complete and build-clean but their toolchains (DXC `as_6_5`, `metal` → `[[object]]`) and runtimes are off-platform and were NOT exercised** — the emitted HLSL/MSL source was inspected by hand and matches each backend's documented shape, but "the source looks right" is not "the driver accepted it."
...
- **D3D12 and Metal are unverified.** Build-clean and written against each backend's documented shape, but neither toolchain nor runtime was exercised. The two highest-risk spots to check first: (a) D3D12 — whether DXC accepts the `groupshared` payload + `DispatchMesh` shape and whether the `space2` root signature builds; (b) Metal — whether `payloadMemoryLength` matches what the compiled `[[object]]` function actually wants, since `payloadStructSize` deliberately over-approximates.
...
### Phase 5 — Amplification / task stage (additive) — **DONE 2026-07-12 (Vulkan verified; D3D12 / Metal off-platform unverified)**
